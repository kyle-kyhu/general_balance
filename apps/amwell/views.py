        ...
        context.update(
            {
                ...
                if file_instance and file_instance.updated_excel_file
                else None,
            }
        )
        return context
...
class BankRecScriptView(View):
    def post(self, request, *args, **kwargs):
        ...
        csv_file_path = os.path.join(
            base_dir, os.path.basename(file_instance.csv_file.name)
        )  # Ensure full path to CSV file
        excel_file_path = os.path.join(
            base_dir, os.path.basename(file_instance.excel_file.name)
        )  # Ensure full path to Excel file
        ...
        try:
            ...
            result = subprocess.run(
                ["python", script_path, csv_file_path, excel_file_path],
                check=True,
                env=env,
                capture_output=True,
                text=True,
