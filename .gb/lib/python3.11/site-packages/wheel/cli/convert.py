...
wininst_re = re.compile(r"\.(?P<platform>win32|win-amd64)(?:-(?P<pyver>py\d\.\d))?\.exe$")
    ...
    for pat in files:
        for archive in iglob(pat):
            ...
            dest_path = Path(dest_dir) / (
                f"{source.name}-{source.version}-{source.pyver}-{source.abi}" f"-{source.platform}.whl"
            )
            with WheelFile(dest_path, "w") as wheelfile:
                ...
                wheel_message.add_header("Root-Is-Purelib", str(source.platform == "any").lower())
                ...
                wheelfile.writestr(
                    ...
                    wheel_message.as_string(policy=serialization_policy).encode("utf-8"),
                )
