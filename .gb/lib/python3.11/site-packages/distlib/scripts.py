...
if os.name == "nt" or (os.name == "java" and os._name == "nt"):
    ...
    WRAPPERS = {r.name: r.bytes for r in finder(DISTLIB_PACKAGE).iterator("") if r.name.endswith(".exe")}
...
class ScriptMaker(object):
    ...
    def _get_shebang(self, encoding, post_interp=b"", options=None):
        ...
        if self.executable:
            ...
            if os.name == "nt":
                ...
                executable = os.path.join(
                    sysconfig.get_config_var("BINDIR"), "python%s" % (sysconfig.get_config_var("EXE"))
                )
            else:
                executable = os.path.join(
        ...
        if (
            sys.platform == "cli" and "-X:Frames" not in post_interp and "-X:FullFrames" not in post_interp
        ):  # pragma: no cover
        ...
        if encoding != "utf-8":
            try:
                ...
                raise ValueError(
                    "The shebang (%r) is not decodable " "from the script encoding (%r)" % (shebang, encoding)
                )
        return shebang
