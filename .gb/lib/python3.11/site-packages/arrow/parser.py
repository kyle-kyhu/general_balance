...
class DateTimeParser:
    ...
    _TIME_RE: ClassVar[Pattern[str]] = re.compile(r"^(\d{2})(?:\:?(\d{2}))?(?:\:?(\d{2}))?(?:([\.\,])(\d+))?$")
    _WEEK_DATE_RE: ClassVar[Pattern[str]] = re.compile(r"(?P<year>\d{4})[\-]?W(?P<week>\d{2})[\-]?(?P<day>\d)?")
    ...
    def __init__(self, locale: str = DEFAULT_LOCALE, cache_size: int = 0) -> None:
        ...
        self._input_re_map.update(
            {
                "MMMM": self._generate_choice_re(self.locale.month_names[1:], re.IGNORECASE),
                "MMM": self._generate_choice_re(self.locale.month_abbreviations[1:], re.IGNORECASE),
                "Do": re.compile(self.locale.ordinal_day_re),
                "dddd": self._generate_choice_re(self.locale.day_names[1:], re.IGNORECASE),
                "ddd": self._generate_choice_re(self.locale.day_abbreviations[1:], re.IGNORECASE),
                "d": re.compile(r"[1-7]"),
                "a": self._generate_choice_re((self.locale.meridians["am"], self.locale.meridians["pm"])),
    ...
    def parse_iso(self, datetime_string: str, normalize_whitespace: bool = False) -> datetime:
        ...
        if has_time:
            ...
            time_parts = re.split(r"[\+\-Z]", time_string, maxsplit=1, flags=re.IGNORECASE)
    ...
    def parse(
        ...
        try:
            ...
            raise ParserMatchError(f"Failed to generate regular expression pattern: {e}.")

        match = fmt_pattern_re.search(datetime_string)

        if match is None:
            raise ParserMatchError(f"Failed to match {fmt!r} when parsing {datetime_string!r}.")
        ...
        for token in fmt_tokens:
            ...
            if value is None:
                raise ParserMatchError(f"Unable to find a match group for the specified token {token!r}.")
    ...
    def _generate_pattern_re(self, fmt: str) -> Tuple[List[_FORMAT_TYPE], Pattern[str]]:
        ...
        bounded_fmt_pattern = r"{}{}{}".format(starting_word_boundary, final_fmt_pattern, ending_word_boundary)
    ...
    def _build_datetime(parts: _Parts) -> datetime:
        ...
        if day_of_year is not None:
            ...
            if _year is None:
                raise ParserError("Year component is required with the DDD and DDDD tokens.")

            if month is not None:
                raise ParserError("Month component is not allowed with the DDD and DDDD tokens.")

            date_string = f"{_year}-{day_of_year}"
            try:
                dt = datetime.strptime(date_string, "%Y-%j")
            except ValueError:
                raise ParserError(f"The provided day of year {day_of_year!r} is invalid.")
        ...
        if hour == 24:
            ...
            if parts.get("microsecond", 0) != 0:
                raise ParserError("Midnight at the end of day must not contain microseconds")
            hour = 0
            day_increment = 1
        else:
            day_increment = 0
...
class TzinfoParser:
    _TZINFO_RE: ClassVar[Pattern[str]] = re.compile(r"^(?:\(UTC)*([\+\-])?(\d{2})(?:\:?(\d{2}))?")
