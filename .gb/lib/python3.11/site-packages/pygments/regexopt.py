...
def regex_opt_inner(strings, open_paren):
    ...
    if suffix:
        ...
        return (
            open_paren + regex_opt_inner(sorted(s[:-slen] for s in strings), "(?:") + escape(suffix[::-1]) + close_paren
        )
    # recurse on common 1-string prefixes
    # print '-> last resort'
    return (
        open_paren
        + "|".join(regex_opt_inner(list(group[1]), "") for group in groupby(strings, lambda s: s[0] == first[0]))
        + close_paren
    )
