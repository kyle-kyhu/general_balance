...
class RebolLexer(RegexLexer):
    ...
    def word_callback(lexer, match):
        ...
        elif re.match(r"(halt|quit|do|load|q|recycle|call|run|ask|parse|view|unview|" r"return|exit|break)$", word):
    ...
    tokens = {
        "root": [
            ...
            (r"\d+[\-/][0-9a-z]+[\-/]\d+(\/\d+\:\d+((\:\d+)?" r"([.\d+]?([+-]?\d+:\d+)?)?)?)?", String.Other),  # date
...
class RedLexer(RegexLexer):
    ...
    def word_callback(lexer, match):
        ...
        if re.match(".*:$", word):
            ...
            r"(yes|on|no|off|true|false|tab|cr|lf|newline|escape|slash|sp|space|null|" r"none|crlf|dot|null-byte)$",
            word,
        ):
            ...
            yield match.start(), Keyword, word
        elif re.match(r"(\+|-\*\*|-|\*\*|//|/|\*|and|or|xor|=\?|===|==|=|<>|<=|>=|" r"<<<|>>>|<<|>>|<|>%)$", word):
            yield match.start(), Operator, word
        elif re.match(r".*\!$", word):
            yield match.start(), Keyword.Type, word
        elif re.match("'.*", word):
    ...
    tokens = {
        "root": [
            ...
            (r"2#\{", Number.Hex, "bin2"),
            (r"64#\{[0-9a-z+/=\s]*\}", Number.Hex),
            (r'([0-9a-f]+)(h)((\s)|(?=[\[\]{}"()]))', bygroups(Number.Hex, Name.Variable, Whitespace)),
            (r'"', String, "string"),
            (r"\{", String, "string2"),
            (r";#+.*\n", Comment.Special),
            (r";\*+.*\n", Comment.Preproc),
