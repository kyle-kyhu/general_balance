...
class JuliaLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            # - <: B or >: B
            (r"([<>]:)(\s*)(" + allowed_variable + r")\b(?![(\[])", bygroups(Operator, Whitespace, Keyword.Type)),
            # - A <: or A >:
            (r"\b(" + allowed_variable + r")(\s*)([<>]:)", bygroups(Keyword.Type, Whitespace, Operator)),
            # operators
            ...
            # chars
            (
                r"'(\\.|\\[0-7]{1,3}|\\x[a-fA-F0-9]{1,3}|\\u[a-fA-F0-9]{1,4}|" r"\\U[a-fA-F0-9]{1,6}|[^\\\'\n])'",
                String.Char,
            ),
            ...
            ("(" + allowed_variable + r")(\{)", bygroups(Keyword.Type, Punctuation), "curly"),
            # - names as part of bare 'where'
            (r"(where)(\s+)(" + allowed_variable + ")", bygroups(Keyword, Whitespace, Keyword.Type)),
            # - curly expressions in general
            (r"(\{)", Punctuation, "curly"),
            # - names as part of type declaration
            (
                r"(abstract|primitive)([ \t]+)(type\b)([\s()]+)(" + allowed_variable + r")",
                bygroups(Keyword, Whitespace, Keyword, Text, Keyword.Type),
            ),
            (
                r"(mutable(?=[ \t]))?([ \t]+)?(struct\b)([\s()]+)(" + allowed_variable + r")",
                bygroups(Keyword, Whitespace, Keyword, Text, Keyword.Type),
            ),
            # macros
            (r"@" + allowed_variable, Name.Decorator),
            (
                words([*OPERATORS_LIST, "..", ".", *DOTTED_OPERATORS_LIST], prefix="@", suffix=operator_suffixes),
                Name.Decorator,
            ),
            # keywords
        ...
        "string": [
            ...
            (r"%[-#0 +]*([0-9]+|[*])?(\.([0-9]+|[*]))?[hlL]?[E-GXc-giorsux%]", String.Interpol),
            (r'[^"$%\\]+', String),
            (r".", String),
        ],
        "tqstring": [
