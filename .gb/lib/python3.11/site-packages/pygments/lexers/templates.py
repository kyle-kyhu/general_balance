...
class SmartyLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(\{)(/?[a-zA-Z_]\w*)(\s*)", bygroups(Comment.Preproc, Name.Function, Text), "smarty"),
        ...
        "smarty": [
            ...
            (r"[0-9](\.[0-9]*)?(eE[+-][0-9])?[flFLdD]?|" r"0[xX][0-9a-fA-F]+[Ll]?", Number),
            (r'"(\\\\|\\[^\\]|[^"\\])*"', String.Double),
            (r"'(\\\\|\\[^\\]|[^'\\])*'", String.Single),
...
class VelocityLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(#\{?)(" + identifier + r")(\}|\b)", bygroups(Comment.Preproc, Name.Function, Comment.Preproc)),
        ...
        "rangeoperator": [(r"\.\.", Operator), include("funcparams"), (r"\]", Operator, "#pop")],
        "funcparams": [
            (r"\$!?\{?", Punctuation, "variable"),
            (r"\s+", Text),
            (r"[,:]", Punctuation),
    ...
    def analyse_text(text):
        ...
        if re.search(r"#\{?foreach\}?\(.+?\).*?#\{?end\}?", text, re.DOTALL):
            rv += 0.15
        if re.search(r"\$!?\{?[a-zA-Z_]\w*(\([^)]*\))?" r"(\.\w+(\([^)]*\))?)*\}?", text):
            rv += 0.01
        return rv
...
class DjangoLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(\{%)(-?\s*)([a-zA-Z_]\w*)", bygroups(Comment.Preproc, Text, Keyword), "block"),
            (r"\{", Other),
        ],
        "varnames": [
            (r"(\|)(\s*)([a-zA-Z_]\w*)", bygroups(Operator, Text, Name.Function)),
            (r"(is)(\s+)(not)?(\s+)?([a-zA-Z_]\w*)", bygroups(Keyword, Text, Keyword, Text, Name.Function)),
        ...
        "var": [(r"\s+", Text), (r"(-?)(\}\})", bygroups(Text, Comment.Preproc), "#pop"), include("varnames")],
        "block": [
            (r"\s+", Text),
...
class MyghtyLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(<&[^|])(.*?)(,.*?)?(&>)", bygroups(Name.Tag, Name.Function, using(PythonLexer), Name.Tag)),
            (r"(?s)(<&\|)(.*?)(,.*?)?(&>)", bygroups(Name.Tag, Name.Function, using(PythonLexer), Name.Tag)),
            (r"</&>", Name.Tag),
...
class MasonLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            (r"\s+", Whitespace),
            (r"(?s)(<%doc>)(.*?)(</%doc>)", bygroups(Name.Tag, Comment.Multiline, Name.Tag)),
            (
                r"(?s)(<%(?:def|method))(\s*)(.*?)(>)(.*?)(</%\2\s*>)",
                bygroups(Name.Tag, Whitespace, Name.Function, Name.Tag, using(this), Name.Tag),
            ),
            (
                r"(?s)(<%(\w+)(.*?)(>))(.*?)(</%\2\s*>)",
                bygroups(Name.Tag, None, None, None, using(PerlLexer), Name.Tag),
            ),
            (r"(?s)(<&[^|])(.*?)(,.*?)?(&>)", bygroups(Name.Tag, Name.Function, using(PerlLexer), Name.Tag)),
            (r"(?s)(<&\|)(.*?)(,.*?)?(&>)", bygroups(Name.Tag, Name.Function, using(PerlLexer), Name.Tag)),
            (r"</&>", Name.Tag),
...
class MakoLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(\$\{)(.*?)(\})", bygroups(Comment.Preproc, using(PythonLexer), Comment.Preproc)),
            (
                r"""(?sx)
                (.+?)                # anything, followed by:
                (?:
...
class CheetahLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(\$)([a-zA-Z_][\w.]*\w)", bygroups(Comment.Preproc, using(CheetahPythonLexer))),
            (r"(?s)(\$\{!?)(.*?)(\})", bygroups(Comment.Preproc, using(CheetahPythonLexer), Comment.Preproc)),
            (
                r"""(?sx)
                (.+?)               # anything, followed by:
                (?:
...
class GenshiMarkupLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            (r"[^<$]+", Other),
            (r"(<\?python)(.*?)(\?>)", bygroups(Comment.Preproc, using(PythonLexer), Comment.Preproc)),
        ...
        "attr": [('"', String, "attr-dstring"), ("'", String, "attr-sstring"), (r"[^\s>]*", String, "#pop")],
...
class JspRootLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"</?jsp:(forward|getProperty|include|plugin|setProperty|useBean).*?>", Keyword),
            (r"[^<]+", Other),
            (r"<", Other),
        ],
        "sec": [
...
class ColdfusionMarkupLexer(RegexLexer):
    ...
    tokens = {
        ...
        "tags": [
            ...
            (r"(?s)(<cfscript.*?>)(.+?)(</cfscript.*?>)", bygroups(Name.Builtin, using(ColdfusionLexer), Name.Builtin)),
        ...
        "cfoutput": [
            (r"[^#<]+", Other),
            (r"(#)(.*?)(#)", bygroups(Punctuation, using(ColdfusionLexer), Punctuation)),
...
class HandlebarsLexer(RegexLexer):
    ...
    tokens = {
        ...
        "dynamic-partial": [
            (r"\s+", Text),
            (r"\)", Punctuation, "#pop"),
            (r"(lookup)(\s+)(\.|this)(\s+)", bygroups(Keyword, Text, Name.Variable, Text)),
            (r"(lookup)(\s+)(\S+)", bygroups(Keyword, Text, using(this, state="variable"))),
            (r"[\w-]+", Name.Function),
            include("generic"),
        ],
        "variable": [
...
class LiquidLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            # output tags
            (r"(\{\{)(\s*)([^\s}]+)", bygroups(Punctuation, Whitespace, using(this, state="generic")), "output"),
            (r"\{", Text),
        ],
        "tag-or-block": [
            ...
            (
                r"(capture)(\s+)([^\s%]+)(\s*)(%\})",
                bygroups(Name.Tag, Whitespace, using(this, state="variable"), Whitespace, Punctuation),
                "#pop",
            ),
        ...
        "filter-markup": [(r"\|", Punctuation, "#pop"), include("end-of-tag"), include("default-param-markup")],
        "condition": [
            ...
            (
                r'([\w.\'"]+)(\s+)(contains)(\s+)([\w.\'"]+)',
                bygroups(
                    using(this, state="generic"), Whitespace, Operator.Word, Whitespace, using(this, state="generic")
                ),
            ),
            include("generic"),
            include("whitespace"),
        ...
        "variable": [(r"[a-zA-Z_]\w*", Name.Variable), (r"(?<=\w)\.(?=\w)", Punctuation)],
...
class TwigLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (
                r"(\{%)(-?\s*)(verbatim)(\s*-?)(%\})(.*?)" r"(\{%)(-?\s*)(endverbatim)(\s*-?)(%\})",
                bygroups(
                    Comment.Preproc,
                    Text,
                    Keyword,
            ...
            (r"(\{%)(-?\s*)([a-zA-Z_]\w*)", bygroups(Comment.Preproc, Text, Keyword), "tag"),
            (r"\{", Other),
        ],
        "varnames": [
            (rf"(\|)(\s*)({_ident_inner})", bygroups(Operator, Text, Name.Function)),
            (rf"(is)(\s+)(not)?(\s*)({_ident_inner})", bygroups(Keyword, Text, Keyword, Text, Name.Function)),
...
class Angular2Lexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"([([]+)([\w:.-]+)([\])]+)(\s*)", bygroups(Punctuation, Name.Attribute, Punctuation, Text)),
            # *ngIf="..."; #f="ngForm"
            (r"([*#])([\w:.-]+)(\s*)(=)(\s*)", bygroups(Punctuation, Name.Attribute, Text, Operator, Text), "attr"),
