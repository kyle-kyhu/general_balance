...
from pygments.lexers._postgres_builtins import KEYWORDS, DATATYPES, PSEUDO_TYPES, PLPGSQL_KEYWORDS, EXPLAIN_KEYWORDS
...
class PostgresLexer(PostgresBase, RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(" + "|".join(s.replace(" ", r"\s+") for s in DATATYPES + PSEUDO_TYPES) + r")\b", Name.Builtin),
...
class PlPgsqlLexer(PostgresBase, RegexLexer):
    ...
    for i, pattern in enumerate(tokens["root"]):
        if pattern[1] == Keyword:
            tokens["root"][i] = (words(KEYWORDS + PLPGSQL_KEYWORDS, suffix=r"\b"), Keyword)
            del i
            break
    else:
...
re_message = re.compile(r"((?:DEBUG|INFO|NOTICE|WARNING|ERROR|" r"FATAL|HINT|DETAIL|CONTEXT|LINE [0-9]+):)(.*?\n)")
...
class PostgresExplainLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(InitPlan|SubPlan)( )(\d+)( )", bygroups(Keyword, Whitespace, Number.Integer, Whitespace), "init_plan"),
            (
                words(
                    (
                        "Sort Method",
            ...
            (r"(Trigger)( )(\S*)(:)( )", bygroups(Comment.Preproc, Whitespace, Name.Variable, Punctuation, Whitespace)),
        ],
        "expression": [
