...
class AtomsLexer(RegexLexer):
    ...
    tokens = {
        ...
        "ordered_values": [
            ...
            # ISO 8601 duration
            (
                r"P((\d*(\.\d+)?[YyMmWwDd]){1,3}(T(\d*(\.\d+)?[HhMmSs]){,3})?|" r"T(\d*(\.\d+)?[HhMmSs]){,3})",
                Literal.Date,
            ),
...
class CadlLexer(AtomsLexer):
    ...
    tokens = {
        ...
        "root": [
            ...
            (r"(\{)(\s*)(/[^}]+/)(\s*)(\})", bygroups(Punctuation, Whitespace, String.Regex, Whitespace, Punctuation)),
            # regex in slot or as string constraint
            (
                r"(\{)(\s*)(\^[^}]+\^)(\s*)(\})",
            ...
            # for cardinality etc
            (
                r"(\{)((?:\d+\.\.)?(?:\d+|\*))" r"((?:\s*;\s*(?:ordered|unordered|unique)){,2})(\})",
...
class AdlLexer(AtomsLexer):
    ...
    tokens = {
        ...
        "root": [
            (r"^(archetype|template_overlay|operational_template|template|" r"speciali[sz]e)", Generic.Heading),
            (
                r"^(language|description|ontology|terminology|annotations|"
                r"component_terminologies|revision_history)[ \t]*\n",
                Generic.Heading,
