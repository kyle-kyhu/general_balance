...
class CAmkESLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (words(("dma_pool", "from_access", "to_access"), suffix=r"\b"), Keyword.Reserved),
            # CAmkES-level include
            (r'(import)(\s+)((?:<[^>]*>|"[^"]*");)', bygroups(Comment.Preproc, Whitespace, Comment.Preproc)),
            # C-level include
            (r'(include)(\s+)((?:<[^>]*>|"[^"]*");)', bygroups(Comment.Preproc, Whitespace, Comment.Preproc)),
            # Literals
...
class CapDLLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"[<>\[(){},:;=\]]", Punctuation),
            (r"\.\.", Punctuation),
            (words(("arch", "arm11", "caps", "child_of", "ia32", "irq", "maps", "objects"), suffix=r"\b"), Keyword),
            (
                words(
                    (
                        "aep",
            ...
            (words(("cspace", "vspace", "reply_slot", "caller_slot", "ipc_buffer_slot"), suffix=r"\b"), Number),
            # Identifiers
            (r"[a-zA-Z_][-@\.\w]*", Name),
        ],
    }
