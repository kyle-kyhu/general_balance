...
class PerlLexer(RegexLexer):
    ...
    tokens = {
        ...
        "root": [
            ...
            (
                r"(format)(\s+)(\w+)(\s*)(=)(\s*\n)",
                bygroups(Keyword, Whitespace, Name, Whitespace, Punctuation, Whitespace),
                "format",
            ),
            ...
            (r"(q|qq|qw|qr|qx)([\W_])(.|\n)*?\2", String.Other),
            (r"(package)(\s+)([a-zA-Z_]\w*(?:::[a-zA-Z_]\w*)*)", bygroups(Keyword, Whitespace, Name.Namespace)),
            (r"(use|require|no)(\s+)([a-zA-Z_]\w*(?:::[a-zA-Z_]\w*)*)", bygroups(Keyword, Whitespace, Name.Namespace)),
...
class Perl6Lexer(ExtendedRegexLexer):
    ...
    tokens = {
        "common": [
            ...
            (r"m\w+(?=\()", Name),
            (
                r"(?:m|ms|rx)\s*(?P<adverbs>:[\w\s:]+)?\s*(?P<delimiter>(?P<first_char>[^\w:\s])" r"(?P=first_char)*)",
    ...
    def analyse_text(text):
        ...
        if re.search(r"(?:my|our|has)\s+(?:" + Perl6Lexer.PERL6_IDENTIFIER_RANGE + r"+\s+)?[$@%&(]", text):
            rating = 0.8
            saw_perl_decl = True
