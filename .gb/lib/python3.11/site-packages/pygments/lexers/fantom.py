...
class FantomLexer(RegexLexer):
    ...
    tokens = {
        ...
        "fieldKeywords": [(r"\b(abstract|const|final|native|override|static|virtual|" r"readonly)\b", Keyword)],
        "otherKeywords": [
            (
                words(
                    (
        ...
        "root": [
            ...
            (r"(class|mixin)(\s+)(\w+)", bygroups(Keyword, Whitespace, Name.Class), "inheritance"),  # Inheritance list
            # Type var := val
            (
                s(r"($type)([ \t]+)($id)(\s*)(:=)"),
                bygroups(using(this, state="inType"), Whitespace, Name.Variable, Whitespace, Operator),
            ),
            # var := val
            ...
            (
                s(
                    r"($type)([ \t]+)"  # Return type and whitespace
                    r"($id)(\s*)(\()"
                ),  # method name + open brace
                bygroups(using(this, state="inType"), Whitespace, Name.Function, Whitespace, Punctuation),
                "insideMethodDeclArgs",
            ),
            # ArgType argName,
            (
                s(r"($type)(\s+)($id)(\s*)(,)"),
                bygroups(using(this, state="inType"), Whitespace, Name.Variable, Whitespace, Punctuation),
            ),
            # ArgType argName)
            ...
            # Type var
            (s(r"($type)([ \t]+)($id)"), bygroups(using(this, state="inType"), Whitespace, Name.Variable)),
        ...
        "using": [
            ...
            (r"(\[)(\w+)(\])", bygroups(Punctuation, Comment.Special, Punctuation)),  # ffi
            (r"(\")?([\w.]+)(\")?", bygroups(Punctuation, Name.Namespace, Punctuation)),  # podname
            (r"::", Punctuation, "usingClass"),
            default("#pop"),
        ],
        "usingClass": [
            (r"[ \t]+", Whitespace),  # consume whitespaces
            (r"(as)(\s+)(\w+)", bygroups(Keyword.Declaration, Whitespace, Name.Class), "#pop:2"),
            (r"[\w$]+", Name.Class),
            default("#pop:2"),  # jump out to root state
        ],
        "facet": [(r"\s+", Whitespace), (r"\{", Punctuation, "facetFields"), default("#pop")],
        "facetFields": [
            include("comments"),
            include("literals"),
            include("operators"),
