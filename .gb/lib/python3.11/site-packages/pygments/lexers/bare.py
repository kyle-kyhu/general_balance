...
class BareLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(type)(\s+)([A-Z][a-zA-Z0-9]+)(\s+)", bygroups(Keyword, Whitespace, Name, Whitespace), "typedef"),
            (r"(enum)(\s+)([A-Z][a-zA-Z0-9]+)(\s+\{)", bygroups(Keyword, Whitespace, Name.Class, Whitespace), "enum"),
            (r"#.*?$", Comment),
            (r"\s+", Whitespace),
        ],
        "struct": [
