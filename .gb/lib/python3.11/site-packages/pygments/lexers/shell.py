...
class BatchLexer(RegexLexer):
    ...
    def _make_begin_state(
        ...
        return [
            ...
            (rf"(call)({_space}?)(:)", bygroups(Keyword, using(this, state="text"), Punctuation), f"call{suffix}"),
            ...
            (
                r'(set{}){}((?:/p)?){}((?:(?:(?:\^[{}]?)?[^"{}{}^={}]|' r'\^[{}]?[^"=])+)?)((?:(?:\^[{}]?)?=)?)'.format(
                    _keyword_terminator, set_space, set_space, _nl, _nl, _punct, ")" if compound else "", _nl, _nl
                ),
                bygroups(
                    Keyword,
                    using(this, state="text"),
    ...
    def _make_arithmetic_state(
        compound, _nl=_nl, _punct=_punct, _string=_string, _variable=_variable, _ws=_ws, _nlws=_nlws
    ):
        op = r"=+\-*/!~"
        state = []
        if compound:
        ...
        state += [
            ...
            (
                r'({}|{}|(\^[{}]?)?[^(){}%\^"{}{}]|\^[{}]?{})+'.format(
                    _string, _variable, _nl, op, _nlws, _punct, _nlws, r"[^)]" if compound else r"[\w\W]"
                ),
                using(this, state="variable"),
            ),
            (r"(?=[\x00|&])", Text, "#pop"),
    ...
    def _make_call_state(compound, _label=_label, _label_compound=_label_compound):
        ...
        if compound:
            state.append((r"(?=\))", Text, "#pop"))
        state.append(
            (r"(:?)(%s)" % (_label_compound if compound else _label), bygroups(Punctuation, Name.Label), "#pop")
        )
        return state
    ...
    tokens = {
        ...
        "variable-or-escape": [(_variable, Name.Variable), (rf"%%|\^[{_nl}]?(\^!|[\w\W])", String.Escape)],
        "string": [
            (r'"', String.Double, "#pop"),
        ...
        "for": [
            (
                rf"({_space})(in)({_space})(\()",
                bygroups(using(this, state="text"), Keyword, using(this, state="text"), Punctuation),
                "#pop",
            ),
            include("follow"),
        ],
        "for2": [
            (r"\)", Punctuation),
            (rf"({_space})(do{_token_terminator})", bygroups(using(this, state="text"), Keyword), "#pop"),
            (rf"[{_nl}]+", Text),
            include("follow"),
        ],
        "for/f": [
        ...
        "if": [
            ...
            (
                ...
                bygroups(Keyword, using(this, state="text"), using(this, state="variable")),
                "#pop",
            ),
            (rf"(exist{_token_terminator})({_space}{_stoken})", bygroups(Keyword, using(this, state="text")), "#pop"),
            (
                rf"({_number}{_space})({_opword})({_space}{_number})",
                bygroups(using(this, state="arithmetic"), Operator.Word, using(this, state="arithmetic")),
                "#pop",
            ),
        ...
        "if2": [
            (
                ...
                bygroups(using(this, state="text"), Operator, using(this, state="text")),
                "#pop",
            ),
            (
                rf"({_space})({_opword})({_space}{_stoken})",
                bygroups(using(this, state="text"), Operator.Word, using(this, state="text")),
                "#pop",
            ),
        ],
        "(?": [
        ...
        "else?": [(_space, using(this, state="text")), (rf"else{_token_terminator}", Keyword, "#pop"), default("#pop")],
    }
...
class ExeclineLexer(RegexLexer):
    ...
    tokens = {
        ...
        "curly": [(r"\}", String.Interpol, "#pop"), (r"[\w#@]+", Name.Variable), include("root")],
    }
