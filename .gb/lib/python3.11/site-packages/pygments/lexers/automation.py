...
class AutoItLexer(RegexLexer):
    ...
    tokens = {
        ...
        "commands": [
            (r"(?i)(\s*)({})\b".format("|".join(keywords)), bygroups(Text, Name.Builtin)),
        ],
        "builtInFunctions": [
            (r"(?i)({})\b".format("|".join(functions)), Name.Function),
        ],
        "builtInMarcros": [
            (r"(?i)({})\b".format("|".join(macros)), Name.Variable.Global),
        ],
        "labels": [
            # sendkeys
            (r"(^\s*)(\{\S+?\})", bygroups(Text, Name.Label)),
        ...
        "dqs": [(r'"', String, "#pop"), include("strings")],
        "sqs": [(r"\'\'|\`([,%`abfnrtv])", String.Escape), (r"'", String, "#pop"), (r"[^'\n]+", String)],
        "garbage": [
            (r"[^\S\n]", Text),
        ],
    }
