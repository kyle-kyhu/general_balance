...
class SparqlLexer(RegexLexer):
    ...
    PN_CHARS_GRP = PN_CHARS_U_GRP + r"\-" + r"0-9" + "\u00b7" + "\u0300-\u036f" + "\u203f-\u2040"
    ...
    tokens = {
        "root": [
            ...
            # prefixed names ::
            (r"(" + PN_PREFIX + r")?(\:)(" + PN_LOCAL + r")?", bygroups(Name.Namespace, Punctuation, Name.Tag)),
            # function names ::
            (
        ...
        "end-of-string": [
            (r"(@)([a-zA-Z]+(?:-[a-zA-Z0-9]+)*)", bygroups(Operator, Name.Function), "#pop:2"),
            (r"\^\^", Operator, "#pop:2"),
            default("#pop:2"),
        ],
    }
...
class TurtleLexer(RegexLexer):
    ...
    tokens = {
        ...
        "end-of-string": [
            (r"(@)([a-zA-Z]+(?:-[a-zA-Z0-9]+)*)", bygroups(Operator, Generic.Emph), "#pop:2"),
...
class ShExCLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            ("(" + BLANK_NODE_LABEL + ")", Name.Label),
            # prefixed names ::
            (r"(" + PN_PREFIX + r")?(\:)(" + PN_LOCAL + ")?", bygroups(Name.Namespace, Punctuation, Name.Tag)),
            # boolean literals ::
            (r"(true|false)", Keyword.Constant),
            # double literals ::
            (r"[+\-]?(\d+\.\d*" + EXPONENT + r"|\.?\d+" + EXPONENT + ")", Number.Float),
