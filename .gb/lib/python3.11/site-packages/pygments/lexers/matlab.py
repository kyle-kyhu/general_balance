...
from pygments.lexer import Lexer, RegexLexer, bygroups, default, words, do_insertions, include
...
class MatlabLexer(RegexLexer):
    ...
    tokens = {
        "expressions": [
            ...
            (rf"(?<!\w)((\d+\.\d+)|(\d*\.\d+)|(\d+\.(?!{_operators})))" r"([eEf][+-]?\d+)?(?!\w)", Number.Float),
        ...
        "propattrs": [
            (r"(\w+)(\s*)(=)(\s*)(\d+)", bygroups(Name.Builtin, Whitespace, Punctuation, Whitespace, Number)),
            (r"(\w+)(\s*)(=)(\s*)([a-zA-Z]\w*)", bygroups(Name.Builtin, Whitespace, Punctuation, Whitespace, Keyword)),
            (r",", Punctuation),
            (r"\)", Punctuation, "#pop"),
            (r"\s+", Whitespace),
            (r".", Text),
    ...
    def analyse_text(text):
        ...
        first_non_comment = next((line for line in text.splitlines() if not re.match(r"^\s*%", text)), "").strip()
        ...
        for match in line_re.finditer(text):
            ...
            if line.startswith(">> "):
                ...
                if line.startswith(" " * line_start):
                    insertions.append((len(curcode), [(0, Generic.Prompt, line[:line_start])]))
...
class OctaveLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (words(builtin_kw + command_kw + function_kw + loadable_kw + mapping_kw, suffix=r"\b"), Name.Builtin),
