...
class MoinWikiLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            # Titles
            (r"^(=+)([^=]+)(=+)(\s*#.+)?$", bygroups(Generic.Heading, using(this), Generic.Heading, String)),
            ...
            # Other Formatting
            (r"\[\[\w+.*?\]\]", Keyword),  # Macro
            (r"(\[[^\s\]]+)(\s+[^\]]+?)?(\])", bygroups(Keyword, String, Keyword)),  # Link
...
class RstLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"^( *\.\.)(\s*)(\[.+\])(.*?)$", bygroups(Punctuation, Text, Name.Tag, using(this, state="inline"))),
            # A substitution def
            (
                r"^( *\.\.)(\s*)(\|.+\|)(\s*)([\w:-]+?)(::)(?:([ \t]*)(.*))",
                bygroups(
                    Punctuation, Text, Name.Tag, Text, Operator.Word, Punctuation, Text, using(this, state="inline")
                ),
            ),
            # Comments
            (r"^ *\.\..*(\n( +.*\n|\n)+)?", Comment),
        ...
        "inline": [
            ...
            (r"(`.+?`)(:[a-zA-Z0-9:-]+?:)?", bygroups(Name.Variable, Name.Attribute)),  # role
            (r"(:[a-zA-Z0-9:-]+?:)(`.+?`)", bygroups(Name.Attribute, Name.Variable)),  # role (content first)
    ...
    def analyse_text(text):
        ...
        if (
            p2 > -1  # has two lines
            and p1 * 2 + 1 == p2  # they are the same length
            and text[p1 + 1] in "-="  # the next line both starts and ends with
            and text[p1 + 1] == text[p2 - 1]
...
class OrgLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            (r"^# .*", Comment.Single),
            # Headings
            (r"^(\* )(COMMENT)( .*)", bygroups(Generic.Heading, Comment.Preproc, Generic.Heading)),
            (r"^(\*\*+ )(COMMENT)( .*)", bygroups(Generic.Subheading, Comment.Preproc, Generic.Subheading)),
            (r"^(\* )(DONE)( .*)", bygroups(Generic.Heading, Generic.Deleted, Generic.Heading)),
            (r"^(\*\*+ )(DONE)( .*)", bygroups(Generic.Subheading, Generic.Deleted, Generic.Subheading)),
            (r"^(\* )(TODO)( .*)", bygroups(Generic.Heading, Generic.Error, Generic.Heading)),
            ...
            # Dynamic blocks
            (r"(?i)^( *#\+begin: *)((?:.|\n)*?)(^ *#\+end: *$)", bygroups(Operator.Word, using(this), Operator.Word)),
            # Comment blocks
            (
                r"(?i)^( *#\+begin_comment *\n)((?:.|\n)*?)(^ *#\+end_comment *$)",
            ...
            (r"(?i)^( *#\+begin_src .*)((?:.|\n)*?)(^ *#\+end_src *$)", bygroups(Operator.Word, Text, Operator.Word)),
            # Other blocks
            (
                r"(?i)^( *#\+begin_\w+)( *\n)((?:.|\n)*?)(^ *#\+end_\w+)( *$)",
            ...
            (r"(?i)^( *:\w+: *\n)((?:.|\n)*?)(^ *:end: *$)", bygroups(Name.Decorator, Comment.Special, Name.Decorator)),
            # Line break operator
            (r"\\\\$", Operator),
            # Deadline, Scheduled, CLOSED
            (r"(?i)^( *(?:DEADLINE|SCHEDULED): )(<.+?> *)$", bygroups(Generic.Error, Literal.Date)),
            (r"(?i)^( *CLOSED: )(\[.+?\] *)$", bygroups(Generic.Deleted, Literal.Date)),
            # Bold
...
class TiddlyWiki5Lexer(RegexLexer):
    ...
    tokens = {
        ...
        "keywords": [
            (
                words(
                    ("\\define", "\\end", "caption", "created", "modified", "tags", "title", "type"),
                    prefix=r"^",
                    suffix=r"\b",
                ),
                Keyword,
        ...
        "inline": [
            ...
            # External links
            (r"(\[{2})([^]\|]+)(\|)([^]\|]+)(\]{2})", bygroups(Text, Name.Tag, Text, Name.Attribute, Text)),
            # Transclusion
            (r"(\{{2})([^}]+)(\}{2})", bygroups(Text, Name.Tag, Text)),
            # URLs
...
class WikitextLexer(RegexLexer):
    ...
    def nowiki_tag_rules(tag_name):
        return [
            (rf"(?i)(</)({tag_name})(\s*)(>)", bygroups(Punctuation, Name.Tag, Whitespace, Punctuation), "#pop"),
            include("entity"),
            include("text"),
        ]
    ...
    tokens = {
        "root": [
            ...
            # Raw URLs
            (r"(?i)\b(?:{}){}{}*".format("|".join(protocols), link_address, link_char_class), Name.Label),
            # Magic links
            (rf"\b(?:RFC|PMID){nbsp_char}+[0-9]+\b", Name.Function.Magic),
            (
                r"""(?x)
                \bISBN {nbsp_char}
                (?: 97[89] {nbsp_dash}? )?
                ...
                [0-9Xx]\b
            """.format(nbsp_char=nbsp_char, nbsp_dash=f"(?:-|{nbsp_char})"),
                Name.Function.Magic,
            ),
            include("list"),
            include("inline"),
        ...
        "inline": [
            ...
            include("entity"),
            # Bold & italic
            (r"('')(''')(?!')", bygroups(Generic.Emph, Generic.EmphStrong), "inline-italic-bold"),
            ...
            (
                ...
                bygroups(
                    ...
                    Name.Namespace,
                    Punctuation,
                    using(this, state=["wikilink-name"]),
                    Punctuation,
                    Name.Label,
                ),
                "medialink-inner",
            ...
            (
                ...
                (\[\[)(?!{}) # Should not contain URLs
                    ...
                    (?: (\#) ([{}]*?) )?
                (\]\])
                """.format("|".join(protocols), title_char.replace("/", ""), title_char, f"{title_char}#"),
                bygroups(
                    Punctuation,
                    Name.Namespace,
                    Punctuation,
            ...
            (
                r"^(:*)(\s*?)(\{\|)([^\n]*)$",
                bygroups(Keyword, Whitespace, Punctuation, using(this, state=["root", "attr"])),
                "table",
            ),
            # HTML tags
            (r"(?i)(<)({})\b".format("|".join(html_tags)), bygroups(Punctuation, Name.Tag), "tag-inner-ordinary"),
            (
                r"(?i)(</)({})\b(\s*)(>)".format("|".join(html_tags)),
                bygroups(Punctuation, Name.Tag, Whitespace, Punctuation),
            ),
            # <nowiki>
            (r"(?i)(<)(nowiki)\b", bygroups(Punctuation, Name.Tag), ("tag-nowiki", "tag-inner")),
            # <pre>
            (r"(?i)(<)(pre)\b", bygroups(Punctuation, Name.Tag), ("tag-pre", "tag-inner")),
            # <categorytree>
            (r"(?i)(<)(categorytree)\b", bygroups(Punctuation, Name.Tag), ("tag-categorytree", "tag-inner")),
            # <hiero>
            (r"(?i)(<)(hiero)\b", bygroups(Punctuation, Name.Tag), ("tag-hiero", "tag-inner")),
            # <math>
            (r"(?i)(<)(math)\b", bygroups(Punctuation, Name.Tag), ("tag-math", "tag-inner")),
            # <chem>
            (r"(?i)(<)(chem)\b", bygroups(Punctuation, Name.Tag), ("tag-chem", "tag-inner")),
            # <ce>
            (r"(?i)(<)(ce)\b", bygroups(Punctuation, Name.Tag), ("tag-ce", "tag-inner")),
            # <charinsert>
            (r"(?i)(<)(charinsert)\b", bygroups(Punctuation, Name.Tag), ("tag-charinsert", "tag-inner")),
            # <templatedata>
            (r"(?i)(<)(templatedata)\b", bygroups(Punctuation, Name.Tag), ("tag-templatedata", "tag-inner")),
            # <gallery>
            (r"(?i)(<)(gallery)\b", bygroups(Punctuation, Name.Tag), ("tag-gallery", "tag-inner")),
            # <graph>
            (r"(?i)(<)(gallery)\b", bygroups(Punctuation, Name.Tag), ("tag-graph", "tag-inner")),
            # <dynamicpagelist>
            (r"(?i)(<)(dynamicpagelist)\b", bygroups(Punctuation, Name.Tag), ("tag-dynamicpagelist", "tag-inner")),
            # <inputbox>
            (r"(?i)(<)(inputbox)\b", bygroups(Punctuation, Name.Tag), ("tag-inputbox", "tag-inner")),
            # <rss>
            (r"(?i)(<)(rss)\b", bygroups(Punctuation, Name.Tag), ("tag-rss", "tag-inner")),
            # <imagemap>
            (r"(?i)(<)(imagemap)\b", bygroups(Punctuation, Name.Tag), ("tag-imagemap", "tag-inner")),
            # <syntaxhighlight>
            (r"(?i)(</)(syntaxhighlight)\b(\s*)(>)", bygroups(Punctuation, Name.Tag, Whitespace, Punctuation)),
            (
            ...
            (
                ...
                bygroups(Punctuation, Name.Tag, Whitespace, using(this, state=["root", "attr"]), Punctuation),
            ),
            # <source>
            (r"(?i)(</)(source)\b(\s*)(>)", bygroups(Punctuation, Name.Tag, Whitespace, Punctuation)),
            (
                r"(?si)(<)(source)\b([^>]*?(?<!/)>.*?)(?=</\2\s*>)",
                bygroups(Punctuation, Name.Tag, handle_syntaxhighlight),
            ),
            ...
            # <score>
            (r"(?i)(</)(score)\b(\s*)(>)", bygroups(Punctuation, Name.Tag, Whitespace, Punctuation)),
            (r"(?si)(<)(score)\b([^>]*?(?<!/)>.*?)(?=</\2\s*>)", bygroups(Punctuation, Name.Tag, handle_score)),
            ...
            # Other parser tags
            (r"(?i)(<)({})\b".format("|".join(parser_tags)), bygroups(Punctuation, Name.Tag), "tag-inner-ordinary"),
            (
        ...
        "inline-italic": [
            ...
            (r"('')(''')(?!')", bygroups(Generic.Emph, Generic.Strong), ("#pop", "inline-bold")),
        ...
        "inline-bold": [
            include("quote-common"),
            (r"(''')('')(?!')", bygroups(Generic.Strong, Generic.Emph), ("#pop", "inline-italic")),
        ...
        "inline-bold-italic": [
            include("quote-common"),
            (r"('')(''')(?!')", bygroups(Generic.EmphStrong, Generic.Strong), "#pop"),
        ...
        "inline-italic-bold": [
            include("quote-common"),
            (r"(''')('')(?!')", bygroups(Generic.EmphStrong, Generic.Emph), "#pop"),
            (r"'''(?!')", Generic.EmphStrong, ("#pop", "inline-italic")),
            (r"''(?!')", Generic.EmphStrong, ("#pop", "inline-bold")),
            include("inline"),
            include("text-bold-italic"),
        ...
        "lc-inner": [
            (
                ...
                (\s* (?:{variants}) \s*) (:)
                """.format(variants="|".join(variant_langs)),
                bygroups(Punctuation, using(this, state=["root", "lc-raw"]), Operator, Name.Label, Punctuation),
            ),
            (r";?\s*?\}-", Punctuation, "#pop"),
            include("inline"),
            include("text"),
        ...
        "replaceable": [
            ...
            # <tvar> legacy syntax
            (r"(?i)(<)(tvar)\b(\|)([^>]*?)(>)", bygroups(Punctuation, Name.Tag, Punctuation, String, Punctuation)),
            (r"</>", Punctuation, "#pop"),
            # <tvar>
            (r"(?i)(<)(tvar)\b", bygroups(Punctuation, Name.Tag), "tag-inner-ordinary"),
            (r"(?i)(</)(tvar)\b(\s*)(>)", bygroups(Punctuation, Name.Tag, Whitespace, Punctuation)),
        ],
        "parameter-inner": [
            (r"\}{3}", Punctuation, "#pop"),
            (r"\|", Punctuation),
        ...
        "template-begin-space": [
            ...
            # Parser functions
            (
                r"(?i)(\#[{}]*?|{})(:)".format(title_char, "|".join(parser_functions_i)),
            ...
            # Templates
            (rf"(?i)([{title_char}]*?)(:)", bygroups(Name.Namespace, Punctuation), ("#pop", "template-name")),
            default(
                ("#pop", "template-name"),
            ),
        ],
        ...
        "table": [
            ...
            (
                ...
                (.*?)$
                """,
                bygroups(Whitespace, Punctuation, using(this, state=["root", "attr"]), Punctuation, Generic.Heading),
            ),
            # Table data
            (
                r"""(?x)
        ...
        "tag-attr": [
            include("replaceable"),
            (r"\s+", Whitespace),
            (r'(=)(\s*)(")', bygroups(Operator, Whitespace, String.Double), "tag-attr-val-2"),
            (r"(=)(\s*)(')", bygroups(Operator, Whitespace, String.Single), "tag-attr-val-1"),
