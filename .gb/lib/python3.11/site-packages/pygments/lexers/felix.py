...
class FelixLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            include("whitespace"),
            # Keywords
            (words(("axiom", "ctor", "fun", "gen", "proc", "reduce", "union"), suffix=r"\b"), Keyword, "funcname"),
            (words(("class", "cclass", "cstruct", "obj", "struct"), suffix=r"\b"), Keyword, "classname"),
            ...
            # Float Literal
            # -- Hex Float
            (r"0[xX]([0-9a-fA-F_]*\.[0-9a-fA-F_]+|[0-9a-fA-F_]+)" r"[pP][+\-]?[0-9_]+[lLfFdD]?", Number.Float),
            # -- DecimalFloat
            (r"[0-9_]+(\.[0-9_]+[eE][+\-]?[0-9_]+|" r"\.[0-9_]*|[eE][+\-]?[0-9_]+)[lLfFdD]?", Number.Float),
        ...
        "whitespace": [
            ...
            (
                r"(#)(\s*)(if)(\s+)(0)",
                bygroups(Comment.Preproc, Whitespace, Comment.Preproc, Whitespace, Comment.Preproc),
                "if0",
            ),
        ...
        "macro": [
            include("comment"),
            (r"(import|include)(\s+)(<[^>]*?>)", bygroups(Comment.Preproc, Whitespace, String), "#pop"),
            (r'(import|include)(\s+)("[^"]*?")', bygroups(Comment.Preproc, Whitespace, String), "#pop"),
            (r"(import|include)(\s+)('[^']*?')", bygroups(Comment.Preproc, Whitespace, String), "#pop"),
            (r"[^/\n]+", Comment.Preproc),
            # (r'/[*](.|\n)*?[*]/', Comment),
