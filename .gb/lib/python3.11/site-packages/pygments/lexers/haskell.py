...
class HaskellLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"0[xX]_*[\da-fA-F](_*[\da-fA-F])*\.[\da-fA-F](_*[\da-fA-F])*" r"(_*[pP][+-]?\d(_*\d)*)?", Number.Float),
        ...
        "import": [
            ...
            (
                ...
                bygroups(Name.Namespace, Whitespace, Keyword, Whitespace, Punctuation),
                "funclist",
            ),
            # import X (functions)
            (r"([" + uni.Lu + r"][\w.]*)(\s+)(\()", bygroups(Name.Namespace, Whitespace, Punctuation), "funclist"),
            # import X
...
class IdrisLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            # Comments
            (r"^(\s*)(%({}))".format("|".join(directives)), bygroups(Whitespace, Keyword.Reserved)),
        ...
        "module": [
            (r"\s+", Whitespace),
            (r"([A-Z][\w.]*)(\s+)(\()", bygroups(Name.Namespace, Whitespace, Punctuation), "funclist"),
...
class AgdaLexer(RegexLexer):
    ...
    tokens = {
        ...
        "module": [(r"\{-", Comment.Multiline, "comment"), (r"[a-zA-Z][\w.\']*", Name, "#pop"), (r"[\W0-9_]+", Text)],
...
class KokaLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (
                r"^(public|private)?(\s+)?(function|fun|val)" r"(\s+)([a-z]\w*|\((?:" + symbols + r"|/)\))",
        ...
        "struct-type": [(r"(?=\((?!,*\)))", Punctuation, "#pop"), include("type")],
        # type started by colon
        "type": [(r"[(\[<]", tokenType, "type-nested"), include("type-content")],
        # type nested in brackets: can contain parameters, comma etc.
        "type-nested": [
            (r"[)\]>]", tokenType, "#pop"),
            (r"[(\[<]", tokenType, "type-nested"),
            (r",", tokenType),
            (r"([a-z]\w*)(\s*)(:)(?!:)", bygroups(Name, Whitespace, tokenType)),  # parameter name
