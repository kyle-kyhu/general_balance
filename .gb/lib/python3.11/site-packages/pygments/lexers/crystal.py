...
class CrystalLexer(ExtendedRegexLexer):
    ...
    def gen_crystalstrings_rules():
        ...
        for lbrace, rbrace, bracecc, name in (
            ...
            states["strings"].append((r"%Q?" + lbrace, String.Other, name + "-intp-string"))
            states[name + "-string"] = (
                [
                    (r"\\[\\" + bracecc + "]", String.Other),
                ]
            ...
            states["strings"].append((r"%[qwi]" + lbrace, String.Other, name + "-string"))
            states[name + "-regex"] = (
                [
    ...
    tokens = {
        "root": [
            ...
            (r"(module|lib)(\s+)([a-zA-Z_]\w*(?:::[a-zA-Z_]\w*)*)", bygroups(Keyword, Whitespace, Name.Namespace)),
            (r"(def|fun|macro)(\s+)((?:[a-zA-Z_]\w*::)*)", bygroups(Keyword, Whitespace, Name.Namespace), "funcname"),
            ...
            # stupid example:
            #   x>=0?n[x]:""
            (r"(0o[0-7]+(?:_[0-7]+)*(?:_?[iu][0-9]+)?)\b(\s*)([/?])?", bygroups(Number.Oct, Whitespace, Operator)),
            (
                r"(0x[0-9A-Fa-f]+(?:_[0-9A-Fa-f]+)*(?:_?[iu][0-9]+)?)\b(\s*)([/?])?",
                bygroups(Number.Hex, Whitespace, Operator),
            ),
            (r"(0b[01]+(?:_[01]+)*(?:_?[iu][0-9]+)?)\b(\s*)([/?])?", bygroups(Number.Bin, Whitespace, Operator)),
            # 3 separate expressions for floats because any of the 3 optional
            # parts makes it a float
            (
                r"((?:0(?![0-9])|[1-9][\d_]*)(?:\.\d[\d_]*)(?:e[+-]?[0-9]+)?" r"(?:_?f[0-9]+)?)(\s*)([/?])?",
            ...
            # annotations
            (r"(@\[)(\s*)([A-Z]\w*(::[A-Z]\w*)*)", bygroups(Operator, Whitespace, Name.Decorator), "in-annot"),
            # this is needed because Crystal attributes can look
            # like keywords (class) or like this: ` ?!?
            (words(CRYSTAL_OPERATORS, prefix=r"(\.|::)"), bygroups(Operator, Name.Operator)),
        ...
        "classname": [
            (r"[A-Z_]\w*", Name.Class),
            (r"(\()(\s*)([A-Z_]\w*)(\s*)(\))", bygroups(Punctuation, Whitespace, Name.Class, Whitespace, Punctuation)),
            default("#pop"),
        ],
        "in-intp": [
        ...
        "string-escaped": [
            ...
            (r'\\([\\abefnrtv#"\']|[0-7]{1,3}|x[a-fA-F0-9]{2}|u[a-fA-F0-9]{4}|u\{[a-fA-F0-9 ]+\})', String.Escape)
        ],
        "string-intp-escaped": [
            include("string-intp"),
