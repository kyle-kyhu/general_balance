...
class CommonLispLexer(RegexLexer):
    ...
    tokens = {
        ...
        "body": [
            ...
            (r"[-+]?(\d*\.\d+([defls][-+]?\d+)?|\d+(\.\d*)?[defls][-+]?\d+)" + terminated, Number.Float),
...
class RacketLexer(RegexLexer):
    ...
    _inexact_simple = rf"(?:{_inexact_simple_no_hashes}|(?:\d+#+(?:\.#*|/\d+#*)?|\.\d+#+|" r"\d+(?:\.\d*#+|/\d+#+)))"
    ...
    tokens = {
        ...
        "datum": [
            ...
            # #d or no prefix
            (rf"(?i){_exact_decimal_prefix}[-+]?\d+(?=[{_delimiters}])", Number.Integer, "#pop"),
            (
                rf"(?i){_exact_decimal_prefix}[-+]?(\d+(\.\d*)?|\.\d+)([deflst][-+]?\d+)?(?=[{_delimiters}])",
                Number.Float,
                "#pop",
            ),
            (
                rf"(?i){_exact_decimal_prefix}[-+]?({_inexact_normal_no_hashes}([-+]{_inexact_normal_no_hashes}?i)?|[-+]{_inexact_normal_no_hashes}?i)(?=[{_delimiters}])",
                Number,
                "#pop",
            ),
            # Inexact without explicit #i
            (
                rf"(?i)(#d)?({_inexact_real}([-+]{_inexact_unsigned}?i)?|[-+]{_inexact_unsigned}?i|{_inexact_real}@{_inexact_real})(?=[{_delimiters}])",
                Number.Float,
                "#pop",
            ),
            ...
            (rf"'|#[s&]|#hash(eqv?)?|#\d*(?={_opening_parenthesis})", Operator, ("#pop", "quoted-datum")),
        ],
        "datum*": [(r"`|,@?", Operator), (_symbol, String.Symbol, "#pop"), (r"[|\\]", Error), default("#pop")],
        "list": [(_closing_parenthesis, Punctuation, "#pop")],
        "unquoted-datum": [
            include("datum"),
            (rf"quote(?=[{_delimiters}])", Keyword, ("#pop", "quoted-datum")),
            (r"`", Operator, ("#pop", "quasiquoted-datum")),
            (rf"quasiquote(?=[{_delimiters}])", Keyword, ("#pop", "quasiquoted-datum")),
        ...
        "quasiquoted-datum": [
            ...
            (rf"unquote(-splicing)?(?=[{_delimiters}])", Keyword, ("#pop", "unquoted-datum")),
        ...
        "string": [
            (r'"', String.Double, "#pop"),
            (r"(?s)\\([0-7]{1,3}|x[\da-fA-F]{1,2}|u[\da-fA-F]{1,4}|" r"U[\da-fA-F]{1,8}|.)", String.Escape),
            (r'[^\\"]+', String.Double),
        ],
    }
...
class JanetLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            # hex number
            (rf"(?x) [+-]? 0x {_hex_unit} \. ({_hex_unit})?", Number.Hex),
            (rf"(?x) [+-]? 0x (\.)? {_hex_unit}", Number.Hex),
            # decimal number
            (rf"(?x) [+-]? {_dec_unit} \. ({_dec_unit})? ({_dec_exp})?", Number.Float),
            (rf"(?x) [+-]? (\.)? {_dec_unit} ({_dec_exp})?", Number.Float),
            # strings and buffers
            (r'@?"', String, "string"),
            ...
            (words(special_forms, prefix=r"(?<=\()", suffix=_token_end), Keyword.Reserved),
            (words(builtin_macros, prefix=r"(?<=\()", suffix=_token_end), Name.Builtin),
            (words(builtin_functions, prefix=r"(?<=\()", suffix=_token_end), Name.Function),
            # other symbols
            (valid_name, Name.Variable),
        ],
        "string": [
