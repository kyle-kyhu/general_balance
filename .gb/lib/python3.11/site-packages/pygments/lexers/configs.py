...
class RegeditLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"\s+", Whitespace),
            (r"[;#].*", Comment.Single),
            (r"(\[)(-?)(HKEY_[A-Z_]+)(.*?\])$", bygroups(Keyword, Operator, Name.Builtin, Keyword)),
            ...
            # Bare keys (includes @)
            (r"(.*?)([ \t]*)(=)([ \t]*)", bygroups(Name.Attribute, Whitespace, Operator, Whitespace), "value"),
        ],
        "value": [
...
class PropertiesLexer(RegexLexer):
    ...
    tokens = {
        ...
        "key": [
            ...
            (r"([^\S\n]*)([:=])([^\S\n]*)", bygroups(Whitespace, Operator, Whitespace), ("#pop", "value")),
...
class Cfengine3Lexer(RegexLexer):
    ...
    tokens = {
        "root": [
            (r"#.*?\n", Comment),
            (r"(body)(\s+)(\S+)(\s+)(control)", bygroups(Keyword, Whitespace, Keyword, Whitespace, Keyword)),
            (
                r"(body|bundle)(\s+)(\S+)(\s+)(\w+)(\()",
                bygroups(Keyword, Whitespace, Keyword, Whitespace, Name.Function, Punctuation),
                "arglist",
            ),
            (r"(body|bundle)(\s+)(\S+)(\s+)(\w+)", bygroups(Keyword, Whitespace, Keyword, Whitespace, Name.Function)),
            (
                r'(")([^"]+)(")(\s+)(string|slist|int|real)(\s*)(=>)(\s*)',
                bygroups(
                    Punctuation, Name.Variable, Punctuation, Whitespace, Keyword.Type, Whitespace, Operator, Whitespace
                ),
            ),
            (r"(\S+)(\s*)(=>)(\s*)", bygroups(Keyword.Reserved, Whitespace, Operator, Text)),
            (r'"', String, "string"),
...
class ApacheConfLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            (r"\s+", Whitespace),
            (r"#(.*\\\n)+.*$|(#.*?)$", Comment),
            (r"(<[^\s>/][^\s>]*)(?:(\s+)(.*))?(>)", bygroups(Name.Tag, Whitespace, String, Name.Tag)),
...
class DockerLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (rf"(HEALTHCHECK)(\s+)(({_lb}--\w+=\w+{_lb})*)", bygroups(Keyword, Whitespace, using(BashLexer))),
            (
                rf"(VOLUME|ENTRYPOINT|CMD|SHELL)(\s+)({_lb})(\[.*?\])",
                bygroups(Keyword, Whitespace, using(BashLexer), using(JsonLexer)),
            ),
            (rf"(LABEL|ENV|ARG)(\s+)(({_lb}\w+=\w+{_lb})*)", bygroups(Keyword, Whitespace, using(BashLexer))),
...
class TOMLLexer(RegexLexer):
    ...
    tokens = {
        ...
        "value": [
            ...
            (r"[+-]?\d[0-9_]*\.\d[0-9_]*([eE][+-]?\d[0-9_]*)?", Number.Float, "#pop"),
            # Infinities and NaN
