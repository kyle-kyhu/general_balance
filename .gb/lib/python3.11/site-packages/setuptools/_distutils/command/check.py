...
with contextlib.suppress(ImportError):
    ...
    class SilentReporter(docutils.utils.Reporter):
        def __init__(
            ...
            self.messages = []
            super().__init__(source, report_level, halt_level, stream, debug, encoding, error_handler)

        def system_message(self, level, message, *children, **kwargs):
            self.messages.append((level, message, children, kwargs))
            return docutils.nodes.system_message(message, *children, level=level, type=self.levels[level], **kwargs)
...
class check(Command):
    ...
    def check_metadata(self):
        ...
        missing = [attr for attr in ("name", "version") if not getattr(metadata, attr, None)]
