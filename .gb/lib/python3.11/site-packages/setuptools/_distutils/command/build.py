...
class build(Command):
    ...
    def finalize_options(self):  # noqa: C901
        if self.plat_name is None:
            ...
            if os.name != "nt":
                raise DistutilsOptionError(
                    "--plat-name only supported on Windows (try " "using './configure --help' on your platform)"
                )
        ...
        if self.build_scripts is None:
            self.build_scripts = os.path.join(self.build_base, "scripts-%d.%d" % sys.version_info[:2])
