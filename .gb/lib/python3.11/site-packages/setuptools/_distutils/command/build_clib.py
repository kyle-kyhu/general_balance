        ...
        for lib in libraries:
            ...
            if not isinstance(name, str):
                raise DistutilsSetupError(
                    "first element of each tuple in 'libraries' " "must be a string (the library name)"
                )

            if "/" in name or (os.sep != "/" and os.sep in name):
                raise DistutilsSetupError(f"bad library name '{lib[0]}': " "may not contain directory separators")

            if not isinstance(build_info, dict):
                raise DistutilsSetupError(
                    "second element of each tuple in 'libraries' " "must be a dictionary (build info)"
                )
