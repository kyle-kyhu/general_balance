...
"""
        ...
        try:
            ...
            os.remove(self.executable_filename("a.out", output_dir=self.output_dir or ""))
        finally:
            for fn in objects:
                os.remove(fn)
        return True
    ...
    def object_filenames(self, source_filenames, strip_dir=False, output_dir=""):
        if output_dir is None:
            output_dir = ""
        return list(self._make_out_path(output_dir, strip_dir, src_name) for src_name in source_filenames)
...
def show_compilers():
    ...
    compilers = sorted(
        ("compiler=" + compiler, None, compiler_class[compiler][2]) for compiler in compiler_class.keys()
    )
...
def new_compiler(plat=None, compiler=None, verbose=False, dry_run=False, force=False):
    ...
    try:
        ...
        raise DistutilsModuleError(f"can't compile C/C++ code: unable to load module '{module_name}'")
    except KeyError:
        raise DistutilsModuleError(
            f"can't compile C/C++ code: unable to find class '{class_name}' " f"in module '{module_name}'"
        )
