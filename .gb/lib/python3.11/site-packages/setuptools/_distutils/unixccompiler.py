...
class UnixCCompiler(CCompiler):
    ...
    def _compile(self, obj, src, ext, cc_args, extra_postargs, pp_opts):
        ...
        try:
            if self.detect_language(src) == "c++":
                self.spawn(compiler_so_cxx + cc_args + [src, "-o", obj] + extra_postargs)
            else:
        ...
        if self._need_link(objects, output_filename):
            ...
            try:
                ...
                linker = (
                    self.linker_exe
                    if building_exe
                    else (self.linker_so_cxx if target_lang == "c++" else self.linker_so)
                )[:]
    ...
    def _library_root(dir):
        ...
        apply_root = (
            sys.platform == "darwin"
            and match
            and (dir.startswith("/System/") or (dir.startswith("/usr/") and not dir.startswith("/usr/local/")))
        )
    ...
    def find_library_file(self, dirs, lib, debug=False):
        ...
        """
        lib_names = (self.library_filename(lib, lib_type=type) for type in "dylib xcode_stub shared static".split())
