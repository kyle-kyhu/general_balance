...
class _ConfigExpander:
    ...
    def _expand_all_dynamic(self, dist: Distribution, package_dir: Mapping[str, str]):
        ...
        obtained_dynamic = {
            field: self._obtain(dist, field, package_dir) for field in self.dynamic if field not in special
        }
    ...
    def _obtain_entry_points(self, dist: Distribution, package_dir: Mapping[str, str]) -> dict[str, dict] | None:
...
def _parse_requirements_list(value):
    return [line for line in value.splitlines() if line.strip() and not line.strip().startswith("#")]
