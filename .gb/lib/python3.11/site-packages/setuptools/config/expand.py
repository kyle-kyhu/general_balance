    ...
    for value in patterns:
        ...
        if any(char in value for char in glob_characters):
            ...
            expanded_values.extend(
                sorted(
                    os.path.relpath(path, root_dir).replace(os.sep, "/") for path in iglob(glob_path, recursive=True)
                )
            )
...
def _find_module(module_name: str, package_dir: Mapping[str, str] | None, root_dir: StrPath) -> str | None:
    ...
    candidates = chain.from_iterable(
        (f"{path_start}{ext}", os.path.join(path_start, f"__init__{ext}")) for ext in all_suffixes()
    )
...
def canonic_data_files(data_files: list | dict, root_dir: StrPath | None = None) -> list[tuple[str, list[str]]]:
