...
class editable_wheel(Command):
    ...
    def _configure_build(self, name: str, unpacked_wheel: StrPath, build_lib: StrPath, tmp_dir: StrPath):
        ...
        egg_info = cast(egg_info_cls, dist.reinitialize_command("egg_info", reinit_subcommands=True))
        egg_info.egg_base = str(tmp_dir)
        egg_info.ignore_egg_info_in_manifest = True

        build = cast(build_cls, dist.reinitialize_command("build", reinit_subcommands=True))
        install = cast(install_cls, dist.reinitialize_command("install", reinit_subcommands=True))
        ...
        install_scripts = cast(install_scripts_cls, dist.get_command_obj("install_scripts"))
...
class _TopLevelFinder:
    ...
    def template_vars(self) -> tuple[str, str, dict[str, str], dict[str, list[str]]]:
        ...
        legacy_namespaces = {
            pkg: find_package_path(pkg, roots, self.dist.src_root or "") for pkg in self.dist.namespace_packages or []
        }
...
def _simple_layout(packages: Iterable[str], package_dir: dict[str, str], project_dir: StrPath) -> bool:
    """Return ``True`` if:
...
def _find_package_roots(
    ...
    pkg_roots: dict[str, str] = {
        pkg: _absolute_root(find_package_path(pkg, package_dir, src_root)) for pkg in sorted(packages)
    }
