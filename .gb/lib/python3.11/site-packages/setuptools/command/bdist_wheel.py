...
def remove_readonly_exc(func: Callable[..., object], path: str, exc: BaseException) -> None:
...
class bdist_wheel(Command):
    ...
    user_options = [
        ...
        (
            "plat-name=",
            "p",
            "platform name to embed in generated filenames " f"[default: {get_platform(None)}]",
        ),
        (
            "keep-temp",
            "k",
        ...
        (
            "compression=",
            None,
            "zipfile compression (one of: {}) [default: 'deflated']".format(", ".join(supported_compressions)),
        ),
        (
            "python-tag=",
            None,
    ...
    def get_tag(self) -> tuple[str, str, str]:
        ...
        if self.root_is_pure:
            ...
            supported_tags = [(t.interpreter, t.abi, plat_name) for t in tags.sys_tags()]
    ...
    def run(self):
        ...
        distinfo_dirname = (
            f"{safer_name(self.distribution.get_name())}-" f"{safer_version(self.distribution.get_version())}.dist-info"
        )
    ...
    def write_wheelfile(self, wheelfile_base: str, generator: str = f"setuptools ({__version__})") -> None:
