...
def distros_for_url(url, metadata=None):
    ...
    if fragment:
        ...
        if match:
            yield from interpret_distro_name(url, match.group(1), metadata, precedence=CHECKOUT_DIST)
...
class HashChecker(ContentChecker):
    pattern = re.compile(r"(?P<hash_name>sha1|sha224|sha384|sha256|sha512|md5)=" r"(?P<expected>[a-f0-9]+)")
