...
def _check_wheel_install(filename, install_dir, install_tree_includes, project_name, version, requires_txt):
    w = Wheel(filename)
...
WHEEL_INSTALL_TESTS: tuple[dict[str, Any], ...] = (
    ...
    dict(
        ...
        setup_kwargs=dict(
            ext_modules=[Record("setuptools.Extension", name="extension", sources=["extension.c"])],
        ),
        install_tree=flatten_tree(
            {
    ...
    dict(
        ...
        file_defs={
            "foobar": {"__init__.py": ("__import__('pkg_resources').declare_namespace(__name__)")},
        },
        setup_kwargs=dict(
...
def test_wheel_is_compatible(monkeypatch):
    def sys_tags():
        return {(t.interpreter, t.abi, t.platform) for t in parse_tag("cp36-cp36m-manylinux1_x86_64")}
