...
def test_apply_pyproject_equivalent_to_setupcfg(url, monkeypatch, tmp_path):
    ...
    if any(getattr(d, "entry_points", None) for d in (dist_toml, dist_cfg)):
        ...
        ep_toml = {(k, *sorted(i.replace(" ", "") for i in v)) for k, v in dist_toml.entry_points.items()}
        ep_cfg = {(k, *sorted(i.replace(" ", "") for i in v)) for k, v in dist_cfg.entry_points.items()}
...
@pytest.mark.parametrize(
    ...
    (
        pytest.param(
            PEP621_EXAMPLE,
            ('Brett Cannon <brett@python.org>, "John X. Ãørçeč" <john@utf8.org>, ' "Γαμα קּ 東 <gama@utf8.org>"),
...
class TestPresetField:
        ...
        [
            ...
            pytest.param(
                *("install_requires", "dependencies", ["six"]),
                marks=[pytest.mark.filterwarnings("ignore:.*install_requires. overwritten")],
            ),
        ],
    )
    ...
    @pytest.mark.parametrize("field,group", [("scripts", "console_scripts"), ("gui-scripts", "gui_scripts")])
