...
def test_recursive_in_package_data_glob(tmpdir_cwd):
    ...
    assert stat.S_ISREG(os.stat("build/lib/path/subpath/subsubpath/data").st_mode), "File is not included"
...
def test_get_outputs(tmpdir_cwd):
    ...
    mapping = {k.replace(os.sep, "/"): v.replace(os.sep, "/") for k, v in build_py.get_output_mapping().items()}
    assert mapping == {
...
def get_outputs(build_py):
    ...
    return {os.path.relpath(x, build_dir).replace(os.sep, "/") for x in build_py.get_outputs()}
