...
class TestPTHFileWriter:
    ...
    def test_many_pth_distributions_merge_together(self, tmpdir):
        ...
        assert pth2.paths[-1] == new_src_path, (
            "and it should match exactly on the last entry actually " "given we append to it in save()"
        )
        # finally,
        assert PthDistributions(pth_path).paths == pth2.paths, (
            "and we should have the exact same list at the end " "with a fresh PthDistributions instance"
        )
    ...
    def test_setup_install_includes_dependencies(self, tmp_path, mock_index):
        ...
        try:
            ...
            assert next(
                ...
                if "not find suitable distribution for" in line and "does-not-exist" in line
            )
        except Exception:
...
class TestSetupRequires:
    ...
    def test_setup_requires_overrides_version_conflict(self, use_setup_cfg):
        ...
        fake_dist = PRDistribution("does-not-matter", project_name="foobar", version="0.0")
        ...
        *already* been imported.
        ...
        with contexts.save_pkg_resources_state():
            with contexts.tempdir() as temp_dir:
                ...
                dist = PRDistribution(foobar_1_dir, project_name="foo.bar", version="0.1")
...
data-requires-python="{dep_2_0_python_requires}">{dep_2_0_sdist}</a><br/>
    ...
    def test_setup_requires_with_find_links_in_setup_cfg(self, monkeypatch, with_dependency_links_in_setup_py):
        monkeypatch.setenv("PIP_RETRIES", "0")
        monkeypatch.setenv("PIP_TIMEOUT", "0")
        with contexts.save_pkg_resources_state():
            with contexts.tempdir() as temp_dir:
    ...
    make_sdist(
        ...
        [
            (
                ...
                """
                ).format(name=distname, version=version, python_requires=python_requires),
            ),
            ("setup.cfg", ""),
        ],
    )
...
class TestScriptHeader:
    ...
    def test_get_script_header_non_ascii_exe(self):
        actual = ei.ScriptWriter.get_header("#!/usr/bin/python", executable=self.non_ascii_exe)
        expected = "#!%s -x\n" % self.non_ascii_exe
        assert actual == expected

    def test_get_script_header_exe_with_spaces(self):
        actual = ei.ScriptWriter.get_header("#!/usr/bin/python", executable='"' + self.exe_with_spaces + '"')
