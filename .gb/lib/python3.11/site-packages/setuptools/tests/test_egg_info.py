...
class TestEggInfo:
    ...
    def test_doesnt_provides_extra(self, tmpdir_cwd, env):
        self._setup_script_with_requires("""install_requires=["spam ; python_version<'3.6'"]""")
        environ = os.environ.copy().update(
            HOME=env.paths["home"],
        )
    ...
    def test_setup_cfg_license_files(self, tmpdir_cwd, env, files, incl_licenses, excl_licenses):
        self._create_project()
        path.build(files)
    ...
    def test_setup_cfg_license_file_license_files(self, tmpdir_cwd, env, files, incl_licenses, excl_licenses):
        self._create_project()
        path.build(files)
    ...
    def test_license_file_attr_pkg_info(self, tmpdir_cwd, env):
        ...
        with open(os.path.join(egg_info_dir, "PKG-INFO"), encoding="utf-8") as fp:
            pkg_info_lines = fp.read().split("\n")
        license_file_lines = [line for line in pkg_info_lines if line.startswith("License-File:")]
    ...
    def test_license_escape(self, tmpdir_cwd, env):
        ...
        self._setup_script_with_requires("license='This is a long license text \\nover multiple lines',")
        code, data = environment.run_setup_py(
            cmd=["egg_info"],
            pypath=os.pathsep.join([env.paths["lib"], str(tmpdir_cwd)]),
            data_stream=1,
