    ...
    for nsp in ns_packages:
        if not dist.has_contents_for(nsp):
            raise DistutilsSetupError(
                "Distribution contains no modules or packages for " + "namespace package %r" % nsp
            )
...
def check_package_data(dist, attr, value):
    ...
    if not isinstance(value, dict):
        raise DistutilsSetupError(
            "{!r} must be a dictionary mapping package names to lists of " "string wildcard patterns".format(attr)
        )
...
class Distribution(_Distribution):
    ...
    def _validate_metadata(self):
        required = {"name"}
        provided = {key for key in vars(self.metadata) if getattr(self.metadata, key, None) is not None}
    ...
    def _finalize_license_files(self) -> None:
        ...
        self.metadata.license_files = list(unique_everseen(self._expand_patterns(patterns)))
            ...
            try:
                ...
                if option in neg_opt and is_string:
                    ...
                    raise DistutilsOptionError(
                        "error in %s: command '%s' has no such option '%s'" % (source, command_name, option)
                    )
            except ValueError as e:
                raise DistutilsOptionError(e) from e
