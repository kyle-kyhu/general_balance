...
class Console:
    ...
    def render(self, renderable: RenderableType, options: Optional[ConsoleOptions] = None) -> Iterable[Segment]:
        ...
        if hasattr(renderable, "__rich_console__") and not isclass(renderable):
            ...
            text_renderable = self.render_str(renderable, highlight=_options.highlight, markup=_options.markup)
        ...
        try:
            ...
            raise errors.NotRenderableError(f"object {render_iterable!r} is not renderable")
        _Segment = Segment
    ...
    def render_str(
        ...
        *,
        ...
        if markup_enabled:
            ...
            rich_text = Text(
                (_emoji_replace(text, default_variant=self._emoji_variant) if emoji_enabled else text),
                justify=justify,
                overflow=overflow,
                style=style,
            )
    ...
    def get_style(self, name: Union[str, Style], *, default: Optional[Union[Style, str]] = None) -> Style:
        ...
        try:
            ...
            raise errors.MissingStyle(f"Failed to get style {name!r}; {error}") from None
        ...
        *objects: Any,
        ...
        with self:
            ...
            if style is None:
                ...
                for renderable in renderables:
                    extend(Segment.apply_style(render(renderable, render_options), self.get_style(style)))
            if new_line_start:
        ...
        *,
        ...
        if json is None:
            ...
            if not isinstance(json, str):
                raise TypeError(f"json must be str. Did you mean print_json(data={json!r}) ?")
    ...
    def export_text(self, *, clear: bool = True, styles: bool = False) -> str:
        ...
        with self._record_buffer_lock:
            if styles:
                text = "".join((style.render(text) if style else text) for text, style, _ in self._record_buffer)
            else:
                text = "".join(segment.text for segment in self._record_buffer if not segment.control)
            if clear:
    ...
    def export_svg(
        ...
        *,
        ...
        def make_tag(name: str, content: Optional[str] = None, **attribs: object) -> str:
            ...
            tag_attribs = " ".join(f'{k.lstrip("_").replace("_", "-")}="{stringify(v)}"' for k, v in attribs.items())
            return f"<{name} {tag_attribs}>{content}</{name}>" if content else f"<{name} {tag_attribs}/>"
        ...
        for y, line in enumerate(Segment.split_and_crop_lines(segments, length=width)):
            ...
            for text, style, _control in line:
                ...
                if style.reverse:
                    has_background = True
                    background = (
                        _theme.foreground_color.hex if style.color is None else style.color.get_truecolor(_theme).hex
                    )
                else:
