...
class RequirementSummary:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return self.key == other.key and self.specifier == other.specifier and self.extras == other.extras
...
def combine_install_requirements(
    ...
    link_attrs = {attr: getattr(source_ireqs[0], attr) for attr in ("link", "original_link")}
...
class BaseResolver(metaclass=ABCMeta):
    ...
    def resolve_hashes(self, ireqs: set[InstallRequirement]) -> dict[InstallRequirement, set[str]]:
...
class LegacyResolver(BaseResolver):
    def __init__(
        ...
        options.features_enabled = omit_list_value(options.features_enabled, "2020-resolver")

    @property
    def constraints(self) -> set[InstallRequirement]:
        return set(self._group_constraints(chain(self.our_constraints, self.their_constraints)))
    ...
    def _group_constraints(self, constraints: Iterable[InstallRequirement]) -> Iterator[InstallRequirement]:
        """
    ...
    def _resolve_one_round(self) -> tuple[bool, set[InstallRequirement]]:
        ...
        removed = {RequirementSummary(t) for t in self.their_constraints} - {RequirementSummary(t) for t in theirs}
    ...
    def get_best_match(self, ireq: InstallRequirement) -> InstallRequirement:
        ...
        log.debug(
            "found candidate {} (constraint was {})".format(format_requirement(best_match), format_specifier(ireq))
        )
    ...
    def _iter_dependencies(self, ireq: InstallRequirement) -> Iterator[InstallRequirement]:
        """
    ...
    def _ireqs_of_dependencies(
        ...
        for dependency_string in dependency_strings:
            yield install_req_from_line(dependency_string, constraint=ireq.constraint, comes_from=ireq)
...
class BacktrackingResolver(BaseResolver):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        options.deprecated_features_enabled = omit_list_value(options.deprecated_features_enabled, "legacy-resolver")
    ...
    def _do_resolve(
        ...
        try:
            resolver.resolve(
                root_reqs=self.constraints + list(compatible_existing_constraints.values()),
            ...
            cause_ireq_names = {strip_extras(key_from_req(cause.requirement)) for cause in cause_exc.causes}
            ...
            for cause_ireq_name in cause_ireq_names:
                ...
                cause_existing_ireq = compatible_existing_constraints.get(cause_ireq_name)
                if cause_existing_ireq is None:
                    raise

                # Remove existing incompatible constraint that causes error
                log.warning(f"Discarding {cause_existing_ireq} to proceed the resolution")
    ...
    def _get_install_requirements(self, resolver_result: Result) -> set[InstallRequirement]:
        ...
        extras_candidates = (candidate for candidate in resolved_candidates if isinstance(candidate, ExtrasCandidate))
    ...
    def _get_install_requirement_from_candidate(
        ...
        pinned_ireq.req.specifier = SpecifierSet(f"{version_pin_operator}{candidate.version}")
