...
def build_project_metadata(
    ...
    *,
    ...
    with _create_project_builder(src_dir, isolated=isolated, quiet=quiet) as builder:
        ...
        requirements = tuple(_prepare_requirements(metadata=metadata, src_file=src_file))
...
def _create_project_builder(src_dir: pathlib.Path, *, isolated: bool, quiet: bool) -> Iterator[build.ProjectBuilder]:
    if quiet:
...
def _prepare_requirements(metadata: PackageMetadata, src_file: pathlib.Path) -> Iterator[InstallRequirement]:
    package_name = _get_name(metadata)
...
def _prepare_build_requirements(
    ...
    for build_target in build_targets:
        for req in builder.get_requires_for_build(build_target):
            result[req].add(f"{package_name} ({src_file}::build-system.backend::{build_target})")
