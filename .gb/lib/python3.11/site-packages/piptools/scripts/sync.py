...
@click.command(name="pip-sync", context_settings={"help_option_names": options.help_option_names})
@options.version
@options.ask
@options.dry_run
@options.force
...
def cli(
    ...
    requirements = flat_map(lambda src: parse_requirements(src, finder=finder, session=session), src_files)
...
def _validate_python_executable(python_executable: str) -> None:
    ...
    if not required_pip_specification.contains(pip_version, prereleases=True):
        msg = "Target python executable '{}' has pip version {} installed. " "Version {} is expected."
        log.error(msg.format(python_executable, pip_version, required_pip_specification))
        sys.exit(2)
...
def _compose_install_flags(
    ...
    for format_control in ("no_binary", "only_binary"):
        ...
        if not formats:
            continue
        result.extend(["--" + format_control.replace("_", "-"), ",".join(sorted(formats))])
