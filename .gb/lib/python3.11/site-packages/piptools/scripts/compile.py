...
def _determine_linesep(strategy: str = "preserve", filenames: tuple[str, ...] = ()) -> str:
    """
...
def cli(
    ...
    if all_build_deps and build_deps_targets:
        raise click.BadParameter("--build-deps-for has no effect when used with --all-build-deps")
    elif all_build_deps:
        build_deps_targets = options.ALL_BUILD_TARGETS

    if only_build_deps and not build_deps_targets:
        raise click.BadParameter("--only-build-deps requires either --build-deps-for or --all-build-deps")
    if only_build_deps and (extras or all_extras):
        raise click.BadParameter("--only-build-deps cannot be used with any of --extra, --all-extras")
    ...
    if output_file.name != "-" and output_file.name in src_files:
        raise click.BadArgumentUsage(f"input and output filenames must not be matched: {output_file.name}")
    ...
    upgrade_install_reqs = {key_from_ireq(install_req): install_req for install_req in upgrade_reqs_gen}
    ...
    if not upgrade and output_file_exists:
        ...
        repository = LocalRequirementsRepository(existing_pins, repository, reuse_hashes=reuse_hashes)
    ...
    constraints.extend(ireq for key, ireq in upgrade_install_reqs.items() if key in primary_packages)
    ...
    linesep = _determine_linesep(strategy=newline, filenames=(output_file.name, *src_files))
    ...
    writer.write(
        ...
        markers={key_from_ireq(ireq): ireq.markers for ireq in constraints if ireq.markers},
        hashes=hashes,
    )
