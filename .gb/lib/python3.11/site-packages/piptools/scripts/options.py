...
trusted_host = click.option(
    "--trusted-host",
    multiple=True,
    help=("Mark this host as trusted, even though it does not have " "valid or any HTTPS; may be used more than once"),
)
...
output_file = click.option(
    ...
    help=(
        "Output file name. Required if more than one input file is given. " "Will be derived from input file otherwise."
    ),
)
...
allow_unsafe = click.option(
    ...
    help=(
        ...
        "--allow-unsafe now to adapt to the upcoming change.".format(", ".join(sorted(UNSAFE_PACKAGES)))
    ),
)
...
reuse_hashes = click.option(
    ...
    is_flag=True,
    default=True,
    help=("Improve the speed of --generate-hashes by reusing the hashes from an " "existing output file."),
)
...
force = click.option("--force", is_flag=True, help="Proceed even if conflicts are found")
