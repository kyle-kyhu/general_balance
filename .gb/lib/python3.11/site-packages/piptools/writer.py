...
MESSAGE_UNSAFE_PACKAGES = comment("# The following packages are considered to be unsafe in a requirements file:")
...
class OutputWriter:
    ...
    def write_header(self) -> Iterator[str]:
        if self.emit_header:
            ...
            yield comment("#")
            compile_command = os.environ.get("CUSTOM_COMPILE_COMMAND") or get_compile_command(self.click_ctx)
    ...
    def _iter_lines(
        ...
        unsafe_requirements = unsafe_requirements or {r for r in results if r.name in unsafe_packages}
        ...
        if packages:
            for ireq in sorted(packages, key=self._sort_key):
                ...
                line = self._format_requirement(ireq, markers.get(key_from_ireq(ireq)), hashes=hashes)
                yield line
            yielded = True
        ...
        if unsafe_requirements:
            ...
            for ireq in sorted(unsafe_requirements, key=self._sort_key):
                ...
                if not self.allow_unsafe:
                    ...
                    line = self._format_requirement(ireq, marker=markers.get(ireq_key), hashes=hashes)
                    yield line
    ...
    def write(
        ...
        try:
            for line in self._iter_lines(results, unsafe_requirements, unsafe_packages, markers, hashes):
                if self.dry_run:
    ...
    def _format_requirement(
        ...
        if hasattr(ireq, "_source_ireqs"):
            required_by |= {
                _comes_from_as_string(src_ireq.comes_from) for src_ireq in ireq._source_ireqs if src_ireq.comes_from
            }
