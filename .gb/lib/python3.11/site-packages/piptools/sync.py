...
def dependency_tree(installed_keys: Mapping[str, Distribution], root_key: str) -> set[str]:
    """
...
def merge(requirements: Iterable[InstallRequirement], ignore_conflicts: bool) -> ValuesView[InstallRequirement]:
        ...
        if ireq.match_markers():
            ...
            if not ignore_conflicts:
                ...
                if existing_ireq:
                    ...
                    if ireq.req and existing_ireq.req and ireq.specifier != existing_ireq.specifier:
...
def diff_key_from_ireq(ireq: InstallRequirement) -> str:
    ...
    if is_url_requirement(ireq):
        if getattr(ireq.req, "name", None) and ireq.link.has_hash:
            return str(direct_url_as_pep440_direct_reference(direct_url_from_link(ireq.link), ireq.req.name))
...
def diff_key_from_req(req: Distribution) -> str:
    ...
    if req.direct_url and isinstance(req.direct_url.info, ArchiveInfo) and req.direct_url.info.hash:
