...
def make_install_requirement(name: str, version: str | Version, ireq: InstallRequirement) -> InstallRequirement:
...
def flat_map(fn: Callable[[_T], Iterable[_S]], collection: Iterable[_T]) -> Iterator[_S]:
...
def get_compile_command(click_ctx: click.Context) -> str:
    ...
    for option_name, value in click_ctx.params.items():
        ...
        if option_long_name == "--config":
            ...
            if str(value) in DEFAULT_CONFIG_FILE_NAMES or parameter_source == ParameterSource.DEFAULT:
                continue
...
def get_required_pip_specification() -> SpecifierSet:
    ...
    requirement = next((r for r in project_dist.requires() if r.name == "pip"), None)  # pragma: no branch
...
def copy_install_requirement(template: InstallRequirement, **extra_kwargs: Any) -> InstallRequirement:
    ...
    ireq.original_link = template.original_link if original_link is None else original_link

    return ireq


def override_defaults_from_config_file(ctx: click.Context, param: click.Parameter, value: str | None) -> Path | None:
    """
...
def _validate_config(
    ...
    compile_cli_params = {param.name: param for param in compile_cli.params if param.name is not None}

    sync_cli_params = {param.name: param for param in sync_cli.params if param.name is not None}
        ...
        for param in associated_params:
            try:
                ...
                raise click.BadOptionUsage(
                    option_name=key,
                    message=(f"Invalid value for config key {key!r}: {value!r}.{os.linesep}" f"Details: {e}"),
                    ctx=click_context,
                ) from e
...
def select_config_file(src_files: tuple[str, ...]) -> Path | None:
    ...
    src_files_as_paths = ((working_directory / src_file).resolve() for src_file in src_files or (".",))
...
def parse_config_file(click_context: click.Context, config_file: Path) -> dict[str, Any]:
    try:
...
def _invert_negative_bool_options_in_config(ctx: click.Context, config: dict[str, Any]) -> dict[str, Any]:
    new_config = {}
    ...
    for key, value in config.items():
        ...
        if new_key.startswith(negative_option_prefix) and long_option not in ONLY_NEGATIVE_OPTIONS:
            new_key = new_key[len(negative_option_prefix) :]

        # Invert negative boolean according to the CLI
        new_value = (
            not value
            if long_option.startswith("--no-") and long_option not in ONLY_NEGATIVE_OPTIONS and isinstance(value, bool)
            else value
        )
