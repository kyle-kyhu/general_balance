...
class PyPIRepository(BaseRepository):
    ...
    def __init__(self, pip_args: list[str], cache_dir: str):
        ...
        self._finder = self.command._build_package_finder(options=options, session=self.session)
    ...
    def find_best_match(self, ireq: InstallRequirement, prereleases: bool | None = None) -> InstallRequirement:
        """
        ...
        matching_candidates = list(
            itertools.chain.from_iterable(candidates_by_version[ver] for ver in matching_versions)
        )
    ...
    def resolve_reqs(
        ...
        with get_build_tracker() as build_tracker, TempDirectory(kind="resolver") as temp_dir, indent_log():
            preparer_kwargs = {
    ...
    def get_dependencies(self, ireq: InstallRequirement) -> set[InstallRequirement]:
        ...
        if not (ireq.editable or is_url_requirement(ireq) or is_pinned_requirement(ireq)):
            raise TypeError(f"Expected url, pinned or editable InstallRequirement, got {ireq}")
    ...
    def _get_project(self, ireq: InstallRequirement) -> Any:
        ...
        """
        package_indexes = (
            PackageIndex(url=index_url, file_storage_domain="") for index_url in self.finder.search_scope.index_urls
        )
    ...
    def _get_download_path(self, ireq: InstallRequirement) -> str:
        ...
        if ireq.link:
            ...
            return os.path.join(self._download_dir, salt[:2], salt[2:4], salt[4:6], salt[6:])
        else:
    ...
    def _get_matching_candidates(self, ireq: InstallRequirement) -> set[InstallationCandidate]:
        """
        ...
        all_candidates = self.find_all_candidates(ireq.name)
        candidates_by_version = lookup_table(all_candidates, key=candidate_version)
        matching_versions = list(ireq.specifier.filter(candidate.version for candidate in all_candidates))
