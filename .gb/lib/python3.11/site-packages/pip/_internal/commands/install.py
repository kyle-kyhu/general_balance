...
class InstallCommand(RequirementCommand):
    ...
    def run(self, options: Values, args: List[str]) -> int:
        ...
        if installing_into_current_environment and not options.override_externally_managed:
            check_externally_managed()
        ...
        if options.target_dir:
            ...
            if (
                # fmt: off
                os.path.exists(options.target_dir) and not os.path.isdir(options.target_dir)
                # fmt: on
            ):
        ...
        try:
            ...
            if options.dry_run:
                would_install_items = sorted(
                    (r.metadata["name"], r.metadata["version"]) for r in requirement_set.requirements_to_install
                )
            ...
            reqs_to_build = [r for r in requirement_set.requirements.values() if should_build_for_install_command(r)]
            ...
            if build_failures:
                raise InstallationError(
                    "ERROR: Failed to build installable wheels for some " "pyproject.toml based projects ({})".format(
    ...
    def _determine_conflicts(self, to_install: List[InstallRequirement]) -> Optional[ConflictDetails]:
        try:
    ...
    def _warn_about_conflicts(self, conflict_details: ConflictDetails, resolver_variant: str) -> None:
        ...
        for project_name in missing:
            ...
            for dependency in missing[project_name]:
                message = f"{project_name} {version} requires {dependency[1]}, " "which is not installed."
