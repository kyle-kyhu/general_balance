...
class RtfFormatter(Formatter):
    ...
    def hex_to_rtf_color(hex_color):
        ...
        return "\\red%d\\green%d\\blue%d;" % (int(hex_color[0:2], 16), int(hex_color[2:4], 16), int(hex_color[4:6], 16))
    ...
    def _lineno_template(self):
        if self.lineno_fontsize != self.fontsize:
            return "{{\\fs{} \\cf{} %s{}}}".format(
                self.lineno_fontsize, self.color_mapping[self.lineno_color], " " * self.lineno_padding
            )

        return "{{\\cf{} %s{}}}".format(self.color_mapping[self.lineno_color], " " * self.lineno_padding)
