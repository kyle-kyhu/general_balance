...
def main_inner(parser, argns):
    ...
    if argns.V:
        print(f"Pygments version {__version__}, (c) 2006-2024 by Georg Brandl, Matthäus " "Chajdas and contributors.")
        return 0
    ...
    if (
        not outfn and sys.platform in ("win32", "cygwin") and fmter.name in ("Terminal", "Terminal256")
    ):  # pragma: no cover
...
def main(args=sys.argv):
    ...
    parser = argparse.ArgumentParser(description=desc, add_help=False, formatter_class=HelpFormatter)
    ...
    lexersel.add_argument(
        "-g",
        action="store_true",
        help="Guess the lexer from the file contents, or pass through " "as plain text if nothing can be guessed.",
    )
    ...
    operation.add_argument("-o", metavar="OUTPUTFILE", help="Where to write the output.  Defaults to standard output.")

    operation.add_argument("INPUTFILE", nargs="?", help="Where to read the input.  Defaults to standard input.")

    flags = parser.add_argument_group("Operation flags")
    flags.add_argument(
        "-v",
        action="store_true",
        help="Print a detailed traceback on unhandled exceptions, which " "is useful for debugging and bug reports.",
    )
    flags.add_argument(
        "-s",
        action="store_true",
    ...
    special_modes.add_argument(
        "-C",
        action="store_true",
        help="Like -N, but print out a lexer name based solely on " "a given content from standard input.",
    )
    special_modes.add_argument(
        "-H",
        action="store",
    ...
    special_modes_group.add_argument(
        "-a", metavar="ARG", help="Formatter-specific additional argument for the -S (print " "style sheet) mode."
    )
