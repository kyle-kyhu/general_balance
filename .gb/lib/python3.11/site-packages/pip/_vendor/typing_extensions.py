...
if sys.version_info >= (3, 13, 0, "beta"):
    ...
    class _SpecialGenericAlias(_special_generic_alias_base, _root=True):
        ...
        def __getitem__(self, params):
            ...
            if self._defaults and len(params) < self._nparams and len(params) + len(self._defaults) >= self._nparams:
    ...
    _NoneType = type(None)
    Generator = _SpecialGenericAlias(collections.abc.Generator, 3, defaults=(_NoneType, _NoneType))
    AsyncGenerator = _SpecialGenericAlias(collections.abc.AsyncGenerator, 2, defaults=(_NoneType,))
    ContextManager = _SpecialGenericAlias(
        contextlib.AbstractContextManager, 2, name="ContextManager", defaults=(typing.Optional[bool],)
    )
    AsyncContextManager = _SpecialGenericAlias(
        contextlib.AbstractAsyncContextManager, 2, name="AsyncContextManager", defaults=(typing.Optional[bool],)
    )
        ...
        def __subclasscheck__(cls, other):
            ...
            if getattr(cls, "_is_protocol", False) and not _allow_reckless_class_checks():
                ...
                if (
                    ...
                    cls.__non_callable_proto_members__ and cls.__dict__.get("__subclasshook__") is _proto_hook
                ):
                    _type_check_issubclass_arg_1(other)
...
if sys.version_info >= (3, 13):
    ...
    def runtime_checkable(cls):
        ...
        if not issubclass(cls, typing.Generic) or not getattr(cls, "_is_protocol", False):
            raise TypeError(f"@runtime_checkable can be only applied to protocol classes," f" got {cls!r}")
        ...
        if isinstance(cls, _ProtocolMeta) or sys.version_info >= (3, 12, 2):
            ...
            for attr in cls.__protocol_attrs__:
                try:
                    ...
                    raise TypeError(
                        f"Failed to determine whether protocol member {attr!r} " "is a method member"
                    ) from e
                else:
                    if not is_callable:
...
if _PEP_728_IMPLEMENTED:
    ...
    class _TypedDictMeta(type):
        def __new__(cls, name, bases, ns, *, total=True, closed=False):
            ...
            if _TAKES_MODULE:
                own_annotations = {
                    n: typing._type_check(tp, msg, module=tp_dict.__module__) for n, tp in own_annotations.items()
                }
            else:
...
# 3.8-3.9
    ...
    def get_origin(tp):
        ...
        if isinstance(tp, _AnnotatedAlias):
            return Annotated
        if isinstance(
            tp, (typing._GenericAlias, _typing_GenericAlias, _BaseGenericAlias, ParamSpecArgs, ParamSpecKwargs)
        ):
...
if hasattr(typing, "NoDefault"):
    ...
    class NoDefaultTypeMeta(type):
        def __setattr__(cls, attr, value):
            ...
            raise TypeError(f"cannot set {attr!r} attribute of immutable type {cls.__name__!r}")
        ...
        def __new__(
            ...
            *constraints,
            ...
            if hasattr(typing, "TypeAliasType"):
                ...
                typevar = typing.TypeVar(
                    name, *constraints, bound=bound, covariant=covariant, contravariant=contravariant
                )
            ...
            def _tvar_prepare_subst(alias, args):
                if typevar.has_default() and alias.__parameters__.index(typevar) == len(args):
...
# 3.10+
    ...
    class ParamSpec(metaclass=_TypeVarLikeMeta):
        ...
        def __new__(
            cls, name, *, bound=None, covariant=False, contravariant=False, infer_variance=False, default=NoDefault
        ):
            if hasattr(typing, "TypeAliasType"):
...
# 3.8-3.9
    ...
    class ParamSpec(list, _DefaultMixin):
        ...
        def __init__(
            self, name, *, bound=None, covariant=False, contravariant=False, infer_variance=False, default=NoDefault
        ):
        ...
        def __new__(cls, name, *, default=NoDefault):
            ...
            def _typevartuple_prepare_subst(alias, args):
                ...
                if var_tuple_index is not None:
                    ...
                    raise TypeError(f"Too few arguments for {alias};" f" actual {alen}, expected at least {plen - 1}")
...
if not hasattr(typing, "TypeVarTuple"):
        ...
        if alen != elen:
            ...
            if hasattr(cls, "__parameters__"):
                ...
                if alen < elen:
                    ...
                    num_default_tv = sum(getattr(p, "__default__", NoDefault) is not NoDefault for p in parameters)
            ...
            raise TypeError(
                f"Too {'many' if alen > elen else 'few'} {things}" f" for {cls}; actual {alen}, expected {expect_val}"
            )
else:
    # Python 3.11+
    ...
    def _check_generic(cls, parameters, elen):
        ...
        if alen != elen:
            ...
            raise TypeError(
                f"Too {'many' if alen > elen else 'few'} arguments" f" for {cls}; actual {alen}, expected {expect_val}"
            )
            ...
            if _is_unpacked_typevartuple(t):
                ...
                if enforce_default_ordering:
                    ...
                    if has_default:
                        ...
                        raise TypeError(
                            f"Type parameter {t!r} without a default" " follows type parameter with a default"
                        )
...
if sys.version_info >= (3, 13):
    ...
    class _NamedTupleMeta(type):
        def __new__(cls, typename, bases, ns):
            ...
            nm_tpl = _make_nmtuple(
                typename, types.items(), defaults=[ns[n] for n in default_names], module=ns["__module__"]
            )
    ...
    def NamedTuple(typename, fields=_marker, /, **kwargs):
        ...
        if fields is _marker:
            ...
            if kwargs:
                raise TypeError(
                    "Cannot pass `None` as the 'fields' parameter " "and also specify fields using keyword arguments"
                )
            else:
                deprecated_thing = "Passing `None` as the 'fields' parameter"
                example = f"`{typename} = NamedTuple({typename!r}, [])`"
    ...
    class TypeAliasType:
        ...
        def __getitem__(self, parameters):
            if not isinstance(parameters, tuple):
                parameters = (parameters,)
            parameters = [
                typing._type_check(item, f"Subscripting {self.__name__} requires a type.") for item in parameters
            ]
