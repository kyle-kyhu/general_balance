...
def get_root() -> str:
    ...
    if inside_git_dir != "false":
        raise FatalError(
            "git toplevel unexpectedly empty! make sure you are not " "inside the `.git` directory of your repository.",
        )
    return root
...
def is_in_merge_conflict() -> bool:
    git_dir = get_git_dir(".")
    return os.path.exists(os.path.join(git_dir, "MERGE_MSG")) and os.path.exists(os.path.join(git_dir, "MERGE_HEAD"))
