...
def filter_by_include_exclude(
    ...
    return (filename for filename in names if include_re.search(filename) if not exclude_re.search(filename))
...
class Classifier:
    ...
    def by_types(
        ...
        for filename in names:
            ...
            if tags >= types and (not types_or or tags & types_or) and not tags & exclude_types:
                yield filename
...
def run(
    ...
    if args.hook_stage in {"prepare-commit-msg", "commit-msg"} and not args.commit_msg_filename:
        logger.error(
            f"`--commit-msg-filename` is required for " f"`--hook-stage {args.hook_stage}`",
        )
        return 1
    # prevent recursive post-checkout hooks (#1418)
    if args.hook_stage == "post-checkout" and environ.get("_PRE_COMMIT_SKIP_POST_CHECKOUT"):
        return 0
    ...
    if args.remote_name and args.remote_url and args.remote_branch and args.local_branch:
