...
def _add_run_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--pre-rebase-branch",
        help=("The branch being rebased, and is not set when  " "rebasing the current branch."),
    )
    parser.add_argument(
        "--commit-msg-filename",
    ...
    parser.add_argument(
        ...
        help=("Source of the commit message " "(typically the second argument to .git/hooks/prepare-commit-msg)"),
    )
    parser.add_argument(
        "--commit-object-name",
        help=("Commit object name " "(typically the third argument to .git/hooks/prepare-commit-msg)"),
    )
    parser.add_argument(
        "--remote-name",
    ...
    parser.add_argument(
        "--is-squash-merge",
        help=("During a post-merge hook, indicates whether the merge was a " "squash merge"),
    )
    parser.add_argument(
        "--rewrite-command",
        help=("During a post-rewrite hook, specifies the command that invoked " "the rewrite"),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    autoupdate_parser.add_argument(
        "--bleeding-edge",
        action="store_true",
        help=("Update to the bleeding edge of `HEAD` instead of the latest " "tagged version (the default behavior)."),
    )
    autoupdate_parser.add_argument(
        "--freeze",
        action="store_true",
    ...
    init_templatedir_parser = _add_cmd(
        "init-templatedir",
        help=("Install hook script in a directory intended for use with " "`git config init.templateDir`."),
    )
    ...
    install_parser.add_argument(
        "--install-hooks",
        action="store_true",
        help=("Whether to install hook environments for all environments " "in the config file."),
    )
    ...
    install_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=("Whether to allow a missing `pre-commit` configuration file " "or exit with a failure code."),
    )
    ...
    try_repo_parser.add_argument(
        "--ref",
        "--rev",
        help=("Manually select a rev to run against, otherwise the `HEAD` " "revision will be used."),
    )
