...
def _hook_installed(hook: Hook) -> bool:
    ...
    return (
        os.path.exists(_state_filename_v2(venv)) or _read_state(venv) == _state(hook.additional_dependencies)
...
def _cloned_repository_hooks(
    ...
    hook_dcts = [_hook(by_id[hook["id"]], hook, root_config=root_config) for hook in repo_config["hooks"]]
    return tuple(
        Hook.create(
            repo_config["repo"],
...
def all_hooks(root_config: dict[str, Any], store: Store) -> tuple[Hook, ...]:
    return tuple(hook for repo in root_config["repos"] for hook in _repository_hooks(repo, store, root_config))
