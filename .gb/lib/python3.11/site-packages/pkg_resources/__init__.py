...
class WorkingSet:
    ...
    def iter_entry_points(self, group: str, name: str | None = None) -> Iterator[EntryPoint]:
        ...
        """
        return (
            entry for dist in self for entry in dist.get_entry_map(group).values() if name is None or name == entry.name
        )
    ...
    def resolve(
        ...
        required_by: collections.defaultdict[Requirement, set[str]] = collections.defaultdict(set)
    ...
    def _resolve_dist(self, req, best, replace_conflicting, env, installer, required_by, to_activate) -> Distribution:
...
class Environment:
    ...
    def obtain(
        self,
        requirement: Requirement,
        installer: Callable[[Requirement], None] | _InstallerType | None | _StrictInstallerType[_DistributionT] = None,
    ) -> Distribution | None:
...
_distribution_finders: dict[type, _DistFinderType[Any]] = _declare_state("dict", "_distribution_finders", {})
...
_namespace_handlers: dict[type, _NSHandlerType[Any]] = _declare_state("dict", "_namespace_handlers", {})
_namespace_packages: dict[str | None, list[str]] = _declare_state("dict", "_namespace_packages", {})
...
def _is_zip_egg(path):
    return path.lower().endswith(".egg") and os.path.isfile(path) and zipfile.is_zipfile(path)
...
def _is_unpacked_egg(path):
    ...
    """
    return path.lower().endswith(".egg") and os.path.isfile(os.path.join(path, "EGG-INFO", "PKG-INFO"))
...
class Distribution:
    ...
    def version(self):
        try:
            ...
            if version is None:
                ...
                msg = ("Missing 'Version:' header and/or {} file at path: {}").format(self.PKG_INFO, path)
    ...
    def requires(self, extras: Iterable[str] = ()) -> list[Requirement]:
        ...
        for ext in extras:
            try:
                ...
                raise UnknownExtra("%s has no such extra feature %r" % (self, ext)) from e
        return deps
    ...
    def __dir__(self):
        return list(set(super().__dir__()) | set(attr for attr in self._provider.__dir__() if not attr.startswith("_")))
    ...
    def get_entry_map(self, group: str | None = None):
        ...
        if not hasattr(self, "_ep_map"):
            self._ep_map = EntryPoint.parse_map(self._get_metadata("entry_points.txt"), self)
...
class DistInfoDistribution(Distribution):
    ...
    def _compute_dependencies(self) -> dict[str | None, list[Requirement]]:
        ...
        for extra in self._parsed_pkg_info.get_all("Provides-Extra") or []:
            ...
            self.__dep_map[s_extra] = [r for r in reqs_for_extra(extra) if r not in common]
