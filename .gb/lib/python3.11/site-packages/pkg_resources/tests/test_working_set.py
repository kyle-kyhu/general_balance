...
def parse_distributions(s):
    ...
    for spec in re.split(r"\n(?=[^\s])", s):
        ...
        if fields:
            ...
            metadata = None
        dist = pkg_resources.Distribution(project_name=name, version=version, metadata=metadata)
        yield dist
...
class FakeInstaller:
    ...
    def __call__(self, req):
        return next(iter(filter(lambda dist: dist in req, self._installable_dists)), None)
...
def parametrize_test_working_set_resolve(*test_list):
    ...
    for test in test_list:
        (
            ...
            expected1,
            expected2,
        ) = (strip_comments(s.lstrip()) for s in textwrap.dedent(test).lstrip().split("\n\n", 5))
