...
def validate_name(ctx, param, value):
    if not value.isidentifier():
        raise click.BadParameter(f"'{value}' is not a valid app name. Please make sure it is a valid identifier.")
    return value
...
@click.argument(
    ...
    type=click.STRING,
    callback=lambda ctx, param, value: [validate_model_name(ctx, param, v) for v in value],
)
@click.option(
    "--config",
...
def _get_team_context(use_teams: bool) -> dict:
    if use_teams:
        ...
        extra_view_args = f", {extra_view_param}: {extra_view_param_type}"  # todo: this is gross
