...
SQL_REGEX = [
    ...
    (r"(?<![\w\])])(\[[^\]\[]+\])", tokens.Name),
    (
        r"((LEFT\s+|RIGHT\s+|FULL\s+)?(INNER\s+|OUTER\s+|STRAIGHT\s+)?" r"|(CROSS\s+|NATURAL\s+)?)?JOIN\b",
        tokens.Keyword,
    ),
    ...
    (r"(LATERAL\s+VIEW\s+)" r"(EXPLODE|INLINE|PARSE_URL_TUPLE|POSEXPLODE|STACK)\b", tokens.Keyword),
