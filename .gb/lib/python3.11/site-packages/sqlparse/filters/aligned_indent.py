...
class AlignedIndentFilter:
    join_words = r"((LEFT\s+|RIGHT\s+|FULL\s+)?" r"(INNER\s+|OUTER\s+|STRAIGHT\s+)?|" r"(CROSS\s+|NATURAL\s+)?)?JOIN\b"
    ...
    def _process_statement(self, tlist):
        if len(tlist.tokens) > 0 and tlist.tokens[0].is_whitespace and self.indent == 0:
            tlist.tokens.pop(0)
    ...
    def _process_case(self, tlist):
        ...
        condition_width = [len(" ".join(map(str, cond))) if cond else 0 for cond, _ in cases]
    ...
    def _split_kwds(self, tlist):
        ...
        while token:
            ...
            # word as aligner
            if token.match(T.Keyword, self.join_words, regex=True) or token.match(T.Keyword, self.by_words, regex=True):
    ...
    def _process_default(self, tlist):
        ...
        for sgroup in tlist.get_sublists():
            ...
            offset_ = 3 if (prev_ and prev_.match(T.Keyword, self.by_words, regex=True)) else 0
