...
def node_version_from_args(args):
    ...
    """
    if args.node == "system":
        out, err = subprocess.Popen(["node", "--version"], stdout=subprocess.PIPE).communicate()
...
def make_parser():
    ...
    if not is_WIN:
        parser.add_argument(
            ...
            dest="jobs",
            default=Config.jobs,
            help="Sets number of parallel commands at node.js compilation. " "The default is 2 jobs.",
        )
        ...
        parser.add_argument(
            "--make", "-m", dest="make_path", metavar="MAKE_PATH", help="Path to make command", default=Config.make
        )
    ...
    parser.add_argument(
        "-l", "--list", dest="list", action="store_true", default=False, help="Lists available node.js versions"
    )
...
def _download_node_file(node_url, n_attempt=3):
    ...
    while n_attempt > 0:
        try:
            ...
            logger.warning("Incomplete read while reading " "from {} - {}".format(node_url, e))
            n_attempt -= 1
            if n_attempt == 0:
                raise e
...
def build_node_from_src(env_dir, src_dir, node_src_dir, args):
    ...
    make_param_values = map(lambda x: getattr(args, x.replace("-", "_")), make_param_names)
    make_opts = [
    ...
    if getattr(sys.version_info, "major", sys.version_info[0]) > 2:
        ...
        if not python2_path:
            raise OSError(
                "Python >=3.0 virtualenv detected, but no python2 " "command (required for building node.js) was found"
            )
    ...
    conf_cmd = ["./configure", "--prefix=%s" % _quote(env_dir)]
    if args.without_ssl:
...
def install_node_wrapped(env_dir, src_dir, args):
    ...
    logger.info(" * Install %s node (%s) " % (src_type, args.node), extra=dict(continued=True))
...
def install_packages(env_dir, args):
    ...
    """
    logger.info(" * Install node.js packages ... ", extra=dict(continued=True))
    ...
    if args.npm == "latest" or real_npm_ver >= "1.0.0":
        cmd = ". " + _quote(activate_path) + " && npm install -g %(pack)s"
    else:
        cmd = ". " + _quote(activate_path) + " && npm install %(pack)s" + " && npm activate %(pack)s"
...
def install_activate(env_dir, args):
    ...
    if args.node == "system":
        ...
        for candidate in ("nodejs", "node"):
            which_node_output, _ = subprocess.Popen(["which", candidate], stdout=subprocess.PIPE, env=env).communicate()
