...
class TemplatesPanel(Panel):
    ...
    def _store_template_info(self, sender, **kwargs):
        ...
        is_debug_toolbar_template = isinstance(template.name, str) and (
            ...
            or template.name.startswith(tuple(self.toolbar.config["SKIP_TEMPLATE_PREFIXES"]))
        )
        if is_debug_toolbar_template:
            return

        kwargs["context"] = [
            context_layer for context_layer in context.dicts if hasattr(context_layer, "items") and context_layer
        ]
            ...
            if pformatted is None:
                ...
                for key, value in context_layer.items():
                    ...
                    # query from SQL Panel
                        ...
                        temp_layer[key] = f"<<{value.__class__.__name__.lower()} of {value.model._meta.label}>>"
                    else:
        ...
        for template_data in self.templates:
            ...
            if self.toolbar.config["SHOW_TEMPLATE_CONTEXT"]:
                if "context_list" not in template_data:
                    template_data["context_list"] = self.process_context_list(template_data.get("context", []))
