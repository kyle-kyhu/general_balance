...
class FormParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "form":
            ...
            self.in_form
            and ((tag == "input" and attrs.get("type") in {"submit", "image"}) or tag == "button")
            and (not attrs.get("form") or attrs.get("form") == "")
        ):
    ...
    def check_invalid_file_form_configuration(self, html_content):
        ...
        for form in parser.forms:
            if (
                ...
                and not any(elem.get("formenctype") == "multipart/form-data" for elem in form["submit_element_attrs"])
            ):
                if form_id := form["form_attrs"].get("id"):
                    alert = self.messages["form_id_missing_enctype"].format(form_id=form_id)
                else:
                    alert = self.messages["form_missing_enctype"]
                self.add_alert({"alert": alert})

        # Check for file inputs that reference a form
        form_attrs_by_id = {form["form_attrs"].get("id"): form["form_attrs"] for form in parser.forms}
        ...
        for attrs in parser.referenced_file_inputs:
            ...
            if form_id and attrs.get("type") == "file":
                ...
                if form_attrs and form_attrs.get("enctype") != "multipart/form-data":
                    alert = self.messages["input_refs_form_missing_enctype"].format(form_id=form_id)
                    self.add_alert({"alert": alert})
