...
class FunctionCall:
    def __init__(self, statobj, func, depth=0, stats=None, id=0, parent_ids=None, hsv=(0, 0.5, 1)):
    ...
    def is_project_func(self):
        ...
        if hasattr(settings, "BASE_DIR"):
            ...
            return file_name.startswith(base_dir) and not any(
                directory in file_name.split(os.path.sep) for directory in ["site-packages", "dist-packages"]
            )
        return None
...
class ProfilingPanel(Panel):
        ...
        if func.depth < max_depth:
            for subfunc in func.subfuncs():
                ...
                if subfunc.stats[3] >= cum_time or (
                    self.capture_project_code and subfunc.is_project_func() and subfunc.stats[3] > 0
                ):
    ...
    def generate_stats(self, request, response):
        ...
        if root_func in self.stats.stats:
            ...
            func_list = []
            cum_time_threshold = root.stats[3] / dt_settings.get_config()["PROFILER_THRESHOLD_RATIO"]
            self.add_node(
                func_list,
                root,
