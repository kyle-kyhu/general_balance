...
def _is_excluded_frame(frame: Any, excluded_modules: Optional[Sequence[str]]) -> bool:
    ...
    if not isinstance(frame_module, str):
        return False
    return any(
        frame_module == excluded_module or frame_module.startswith(excluded_module + ".")
...
def _stack_trace_deprecation_warning() -> None:
    warnings.warn(
        "get_stack() and tidy_stacktrace() are deprecated in favor of" " get_stack_trace()",
        DeprecationWarning,
        stacklevel=2,
    )
...
def tidy_stacktrace(stack: List[stubs.InspectStack]) -> stubs.TidyStackTrace:
    ...
    for frame, path, line_no, func_name, text in (f[:5] for f in stack):
        ...
        frame_locals = pformat(frame.f_locals) if dt_settings.get_config()["ENABLE_STACKTRACES_LOCALS"] else None
...
def get_template_context(node: Node, context: stubs.RequestContext, context_lines: int = 3) -> Dict[str, Any]:
    ...
    for line_num, content in source_lines:
        if start <= line_num <= end:
            debug_context.append({"num": line_num, "content": content, "highlight": (line_num == line)})
        ...
        *,
        ...
        for frame in _stack_frames(skip=skip):
            ...
            if is_source:
                ...
                source_line = linecache.getline(filename, line_no, module_globals).strip()
            else:
                source_line = ""
...
def is_processable_html_response(response):
    ...
    return not getattr(response, "streaming", False) and content_encoding == "" and content_type in _HTML_TYPES
