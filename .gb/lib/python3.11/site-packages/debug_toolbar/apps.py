...
def check_template_config(config):
    ...
    has_app_loaders = loaders is None or "django.template.loaders.app_directories.Loader" in loaders
...
def check_middleware(app_configs, **kwargs):
    ...
    if not debug_toolbar_indexes:
        ...
        errors.append(
            Warning(
                "debug_toolbar.middleware.DebugToolbarMiddleware occurs " "multiple times in MIDDLEWARE.",
                hint="Load debug_toolbar.middleware.DebugToolbarMiddleware only " "once in MIDDLEWARE.",
...
def is_middleware_class(middleware_class, middleware_path):
    try:
        ...
        return
    return inspect.isclass(middleware_cls) and issubclass(middleware_cls, middleware_class)
    ...
    if not panels:
        errors.append(
            Warning(
                ...
                hint="Set DEBUG_TOOLBAR_PANELS to a non-empty list in your " "settings.py.",
...
def js_mimetype_check(app_configs, **kwargs):
    ...
    check_failed = not set(mimetypes.guess_type("toolbar.js")).intersection(javascript_types)
    if check_failed:
        return [
            Warning(
...
def debug_toolbar_installed_when_running_tests_check(app_configs, **kwargs):
    ...
    show_toolbar_changed = dt_settings.get_config()["SHOW_TOOLBAR_CALLBACK"] != CONFIG_DEFAULTS["SHOW_TOOLBAR_CALLBACK"]
    try:
    ...
    if not settings.DEBUG and dt_settings.get_config()["IS_RUNNING_TESTS"] and likely_error_setup:
        return [
            Error(
