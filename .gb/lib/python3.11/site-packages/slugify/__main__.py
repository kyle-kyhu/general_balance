...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    input_group.add_argument("input_string", nargs="*", help="Text to slugify")
    input_group.add_argument("--stdin", action="store_true", help="Take the text from STDIN")
    ...
    parser.add_argument("--max-length", type=int, default=0, help="Output string length, 0 for no limit")
    parser.add_argument(
        "--word-boundary",
        action="store_true",
        default=False,
    ...
    parser.add_argument("--stopwords", nargs="+", help="Words to discount")
    parser.add_argument("--regex-pattern", help="Python regex pattern for disallowed characters")
    parser.add_argument(
        "--no-lowercase", action="store_false", dest="lowercase", default=True, help="Activate case sensitivity"
    )
    parser.add_argument(
        "--replacements", nargs="+", help="""Additional replacement rules e.g. "|->or", "%%->percent"."""
    )
    parser.add_argument("--allow-unicode", action="store_true", default=False, help="Allow unicode characters")
